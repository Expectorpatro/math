...
\begin{theorem}\label{theo:maxminxAx/xx}
	...
	\begin{gather*}
		...
		\max_{x\ne\mathbf{0},\;x\perp\varphi_1,\varphi_2,\varphi_k}\frac{x^TAx}{x^Tx}=\lambda_{k+1}=\varphi_{k+1}^TA\varphi_{k+1}
	\end{gather*}
\end{theorem} 
\begin{proof}
	由\cref{prop:HermitianMatEigen}(3)可知存在一个正交矩阵$Q$使得$Q^{-1}AQ=\operatorname{diag}\{\seq{\lambda}{n}\}=\varLambda$，其中$Q=(\varphi_1^T;\varphi_2^T;\dots;\varphi_n^T)$。根据\cref{prop:OrthogonalUnitaryMatrix}(1)可得：
	\begin{equation*}
		\frac{x^TAx}{x^Tx}=\frac{x^TQ^T\varLambda Qx}{x^TQ^TQx}=\frac{y^T\varLambda y}{y^Ty}=\frac{\sum\limits_{i=1}^{n}\lambda_iy_i^2}{\sum\limits_{i=1}^ny_i^2}\leqslant\lambda_1\frac{\sum\limits_{i=1}^{n}y_i^2}{\sum\limits_{i=1}^ny_i^2}=\lambda_1
	\end{equation*}
	当$y$为$(1,0,0,\dots,0)^T$时第一式取等号，由\cref{theo:SolutionOfSLE2}(2.1)可知关于$x$的非齐次线性方程组$Qx=y$有唯一解，解为$x=Q^{-1}y=\varphi_1$。同理可得第二式。由\cref{prop:OrthogonalUnitarySimilarity}(2)可得当$x\perp\varphi_1,\varphi_2,\dots,\varphi_k$时，有：
	\begin{equation*}
		\frac{x^TAx}{x^Tx}=\frac{x^TQ^T\varLambda Qx}{x^TQ^TQx}=\frac{y^T\varLambda y}{y^Ty}=\frac{\sum\limits_{i=k+1}^{n}\lambda_iy_i^2}{\sum\limits_{i=k+1}^ny_i^2}\leqslant\lambda_1\frac{\sum\limits_{i=k+1}^{n}y_i^2}{\sum\limits_{i=k+1}^ny_i^2}=\lambda_1
	\end{equation*}
