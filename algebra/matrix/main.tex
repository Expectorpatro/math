...
\subsection{初等反射矩阵}
\begin{definition}
	设$\omega\in\mathbb{R}^{n},\;||\omega||=1$，称矩阵$H(\omega)=I_n-2\omega\omega^T$为\gls{ElementaryReflectionMatrix}，也称为\gls{HouseholderTransformation}。
\end{definition}
\begin{note}
...
\begin{theorem}[G约化定理]\label{theo:Givens}
	设$x=(x_1,\dots,x_i,\dots,x_j,\dots,x_n)^T\in\mathbb{R}^{n},\;x_ix_j\ne0$，则平面旋转矩阵$G(i,j,\theta)$使得$G(i,j,\theta)x=(x_1,\dots,x_i',\dots,0,\dots,x_n)^T$，其中$x_i'=\sqrt{x_i^2+x_j^2},\;\theta=\arctan(x_j/x_i)$。
\end{theorem}
\begin{proof}
	计算即可得到：
	\begin{gather*}
