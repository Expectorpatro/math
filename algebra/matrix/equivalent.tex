...
\begin{property}\label{prop:A-}
	...
	\begin{enumerate}
		...
		\item 若$\mathcal{M}(B)\subseteq\mathcal{M}(A),\mathcal{M}(C)\subseteq\mathcal{M}(A^{\top})$，则$C^{\top}A^-B$是唯一的\info{两个唯一性的证明都问题}；
...
\begin{proof}
	...
	\end{equation*}\par
	(4)由\cref{prop:OrthogonalProjectionMat}(2)可知$A(A^{\top}A)^-A^{\top}$是向$\mathcal{M}(A)$的正交投影阵，由\cref{prop:ProjectionTransformation}(7)和\cref{theo:LinearTransformationMatrix}可得$A(A^{\top}A)^-A$是唯一的，与$(A^{\top}A)^-$的选择无关。\par
	(5)此时有：
	\begin{equation*}
	...
	\end{align*}
	所以$B=\mathbf{0}$（考虑$B^{\top}B$主对角线上的元素），于是$A(A^{\top}A)^-A^{\top}A=A$。第二个等式由\cref{prop:Transpose}(4)和第一个等式即可得到。\par
