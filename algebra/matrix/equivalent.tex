...
\begin{property}\label{prop:A-}
	...
	\begin{enumerate}
		...
		\item $A(A^TA)^-A^T$与$(A^TA)^-$的选择无关\info{数域问题}；
...
\begin{proof}
	...
	\end{gather*}
	显然$\frac{\alpha^H}{||\alpha||^2}=\alpha^+$。\par
...
\end{theorem}
\begin{proof}
	由\cref{theo:ConsistentLinearEqCondition}的充分性可知对于给定的这一$A^-$，$A^-\beta$为$Ax=\beta$的一个特解，而由\cref{theo:HomogeneousLinearEq'sGeneralSolution}可知齐次线性方程组$Ax=\mathbf{0}$的通解为$(I_n-A^-A)y$，由\info{链接线性方程组的关系}可得$Ax=\beta$的通解为$x=A^-\beta+(I_n-A^-A)y$。
\end{proof}
\begin{theorem}[结构2]
