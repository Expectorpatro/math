...
\end{definition}
 \begin{theorem}\label{theo:ExistenceOfGeneralizedInverse}
	设非零矩阵$A\in M_{m\times n}(K)$，$\operatorname{rank}(A)=r$。由\cref{prop:EquivalentMatrix}(2)可知：
	\begin{equation*}
		A=P
		\begin{pmatrix}
			I_r & \mathbf{0} \\
...
\begin{proof}
	...
	\begin{equation*}
		...
		\end{pmatrix}
	\end{equation*}
	其中$H,B,C,D$分别为数域$K$上的$r\times r,\;r\times (m-r),\;(n-r)\times r,\;(n-r)\times(m-r)$矩阵。于是：
	\begin{gather*}
		\begin{pmatrix}
			I_r & \mathbf{0} \\
			\mathbf{0} & \mathbf{0}
	...
	\begin{equation*}
		...
		P^{-1}
	\end{equation*}
	由上述证明过程可以发现方程$AXA=A$的解只需满足$H=I_r$，这个过程完全没有涉及到$B,C,D$，于是$B,C,D$可为数域$K$上任意的$r\times r,\;r\times (m-r),\;(n-r)\times r,\;(n-r)\times(m-r)$矩阵，矩阵方程$AXA=A$的解不唯一。
\end{proof}
\begin{property}\label{prop:A-}
	设$A\in M_{m\times n}(K),\;B\in M_{m\times q}(K),\;C\in M_{p\times n}(K)$，$\operatorname{rank}(A)=r$，则广义逆$A^-$具有如下性质：
	\begin{enumerate}
		...
		\item 若$\mathcal{M}(B)\subseteq\mathcal{M}(A),\mathcal{M}(C)\subseteq\mathcal{M}(A^{\top})$，则$C^{\top}A^-B$与$A^-$的选择无关；
		\item $A(A^{\top}A)^-A^{\top}$与$(A^{\top}A)^-$的选择无关；
		\item 若$A$对称，则$(A^-)^{\top}$也是$A$的一个广义逆；
		\item $A(A^{\top}A)^-A^{\top}A=A,\;A^{\top}A(A^{\top}A)^-A^{\top}=A^{\top}$；
		\item 若存在$\alpha$使得$c=A^{\top}\alpha$，则$c^{\top}(A^{\top}A)^-A^{\top}A=c^{\top}$。
	\end{enumerate}
\end{property}
\begin{proof}
	(1)\textbf{充分性：}若$A$可逆，由\cref{prop:InvertibleMatrix}(3.b)可知$r=n$，根据\cref{theo:ExistenceOfGeneralizedInverse}中$A^-$的通解公式可知此时$A^-$唯一。\par
	\textbf{必要性：}若$A^-$唯一，根据\cref{theo:ExistenceOfGeneralizedInverse}可知$r=n$，由\cref{prop:InvertibleMatrix}(3.b)可知此时$A$可逆。\par
	将$A^{-1}$代入方程$AXA=A$发现$A^{-1}$是其的解，由唯一性可知$A^{-}=A^{-1}$。\par
	(2)由\cref{theo:ExistenceOfGeneralizedInverse}可知$\operatorname{rank}(A^-)\geqslant r=\operatorname{rank}(A)$。因为：
	\begin{gather*}
		AA^-=P
		\begin{pmatrix}
			I_r & \mathbf{0} \\
		...
		Q
	\end{gather*}
	根据\cref{prop:InvertibleMatrix}(16)可得$\operatorname{rank}(AA^-)=\operatorname{rank}(A^-A)=\operatorname{rank}(A)=r$。\par
	(3)由已知条件可知存在$D_1,D_2$使得$B=AD_1,\;C=A^{\top}D_2$，于是：
	\begin{equation*}
		C^{\top}A^-B=D_2^{\top}AA^-AD_1=D_2^{\top}AD_1
	\end{equation*}
	若$A$满秩，根据\cref{prop:LinearlyDependent}(4)可知$D_1,D_2$是唯一的，即$C^{\top}A^-B$是唯一的。\par
	若$A$不满秩，由\cref{prop:LinearlyDependent}(4)可知满足已知条件的$D_1,D_2$不唯一。任取与$D_1,D_2$不同的$\tilde{D}_1,\tilde{D}_2$，根据\cref{prop:Transpose}(4)可得：
	\begin{align*}
		&\tilde{D}_2^{\top}A\tilde{D}_1-D_2^{\top}AD_1=\tilde{D}_2^{\top}A(\tilde{D}_1-D_1+D_1)-D_2^{\top}AD_1 \\
		=&\tilde{D}_2^{\top}A(\tilde{D}_1-D_1)+(\tilde{D}_2^{\top}-D_2^{\top})AD_1=\mathbf{0}
	\end{align*}
	即$C^{\top}A^-B$是唯一的。\par
	(4)由\cref{theo:VectorSpaceAAAT}可知存在矩阵$B$使得$A^{\top}=A^{\top}AB$，根据\cref{prop:Transpose}(4)可知：
	\begin{equation*}
		A(A^{\top}A)^{-}A^{\top}=B^{\top}A^{\top}A(A^{\top}A)^{-}A^{\top}AB=B^{\top}A^{\top}AB
	\end{equation*}\par
	若$A$满秩，由\cref{prop:LinearlyDependent}(4)可知$B$是唯一的，所以$A(A^{\top}A)^{-}A^{\top}$是唯一的。\par
	若$A$不满秩，由\cref{prop:LinearlyDependent}(4)可知满足已知条件的$B$不唯一。任取与$B$不同的$\tilde{B}$，根据\cref{prop:Transpose}(4)可得：
	\begin{align*}
		&\tilde{B}^{\top}A^{\top}A\tilde{B}-B^{\top}A^{\top}AB=\tilde{B}^{\top}A^{\top}A(\tilde{B}-B+B)-B^{\top}A^{\top}AB \\
		=&\tilde{B}^{\top}A^{\top}A(\tilde{B}-B)+(\tilde{B}^{\top}-B^{\top})A^{\top}AB=\mathbf{0}
	\end{align*}
	即$A(A^{\top}A)^{-}A^{\top}$是唯一的。\par
	(5)此时有：
	\begin{equation*}
		AXA=A\iff A^{\top}X^{\top}A^{\top}=A^{\top}\iff AX^{\top}A=A
	\end{equation*}\par
	(6)设$B=A(A^{\top}A)^-A^{\top}A-A$，由\cref{prop:Transpose}(4)可得：
	\begin{align*}
		B^{\top}B
		&=\{A^{\top}A[(A^{\top}A)^-]^{\top}A^{\top}-A^{\top}\}[A(A^{\top}A)^-A^{\top}A-A] \\
		&=A^{\top}A[(A^{\top}A)^-]^{\top}A^{\top}A(A^{\top}A)^-A^{\top}A-A^{\top}A[(A^{\top}A)^-]^{\top}A^{\top}A \\
		&\quad-A^{\top}A(A^{\top}A)^-A^{\top}A+A^{\top}A \\
		&=A^{\top}A[(A^{\top}A)^-]^{\top}A^{\top}A-A^{\top}A[(A^{\top}A)^-]^{\top}A^{\top}A-A^{\top}A+A^{\top}A=\mathbf{0}
	\end{align*}
	所以$B=\mathbf{0}$（考虑$B^{\top}B$主对角线上的元素），于是$A(A^{\top}A)^-A^{\top}A=A$。\par
	(7)由(6)和\cref{prop:Transpose}(4)可得：
	\begin{equation*}
		c^{\top}(A^{\top}A)^-A^{\top}A=\alpha^{\top}A(A^{\top}A)^-A^{\top}A=\alpha^{\top}A=c^{\top}\qedhere
	\end{equation*}
\end{proof}
