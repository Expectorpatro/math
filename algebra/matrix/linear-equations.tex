...
\end{definition}
\begin{definition}
	设$A\in K^{m\times n}$，$\mathcal{T}:K^n\longrightarrow K^m$为矩阵$A$诱导的线性映射：
	\begin{equation*}
		\mathcal{T}x=Ax
	\end{equation*}
	定义矩阵$A$的零空间为：
	\begin{equation*}
		\operatorname{Ker}(A)=\{x\in K^n:Ax=\mathbf{0}\}=\operatorname{Ker}(\mathcal{T})
	\end{equation*}
	矩阵$A$的像空间为：
	\begin{equation*}
		\operatorname{Im}(A)=\{Ax:x\in K^n\}=\operatorname{Im}(\mathcal{T})
	\end{equation*}
\end{definition}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State $\mathcal{P} \gets$ set of index of pivot columns in row echelon form, $\mathcal{F} \gets \{1,\dots,n\}\setminus\mathcal{P}$, $r \gets |\mathcal{P}|$
		\If{$r = n$}
