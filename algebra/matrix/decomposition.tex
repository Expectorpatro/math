...
\begin{theorem}\label{theo:Cholesky}
	设$A\in M_{n}(\mathbb{R}^{})$是对称正定矩阵，则存在唯一一个下三角矩阵$L$使得$A=LL^T$，其中$L$的对角线元素均为正数。
\end{theorem}
\begin{proof}
	...
	\begin{equation*}
		A=LDL^T
	\end{equation*}
	因为$A$正定，所以$D$主对角线上的元素都为正数。令$\tilde{L}=L\operatorname{diag}\{\sqrt{u_{11}},\sqrt{u_{22}},\dots,\sqrt{u_{nn}}\}$，则$A=\tilde{L}\tilde{L}^T$。
\end{proof}
\begin{method}
