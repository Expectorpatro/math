...
\begin{proof}
	...
	\textbf{Givens变换：}由\cref{theo:Givens}和\info{正交矩阵的乘积仍然是正交矩阵}可知存在正交矩阵$Q$使得$QA=R$，其中$R$是上三角矩阵，所以$A=Q^{-1}R$。\par
	...
	\begin{equation*}
		...
		\end{pmatrix}
	\end{equation*}
	即可得$H_2H_1$使得$A$的前两列变为上三角的状态。依次重复可得到矩阵序列$\{H_n\}$与$\{A_n\}$，由\info{正交矩阵的乘积仍然是正交矩阵}可知存在正交矩阵$Q$使得$QA=R$，其中$R$是上三角矩阵，所以$A=Q^{-1}R$。\par
