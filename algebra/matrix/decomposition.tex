...
\begin{method}
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}[1]
			\Require Lower triangular matrix $L=(l_{ij})\in M_{n}(K)$, right-hand side vector $b\in K^n$
			\Ensure Solution vector $x\in K^n$ satisfying $Lx=b$
	...
	\begin{algorithm}[H]
		...
		\label{alg:BackwardSubstitution}
		\begin{algorithmic}[1]
			\Require Upper triangular matrix $U=(u_{ij})\in M_{n}(K)$, right-hand side vector $b\in K^n$
			\Ensure Solution vector $x\in K^n$ satisfying $Ux=b$
...
\begin{method}
	...
	\begin{algorithm}[H]
		...
		\label{alg:CholeskyFactorization}
		\begin{algorithmic}[1]
...
\begin{proof}
	...
	\begin{equation*}
		...
		\end{pmatrix}
	\end{equation*}
	因为$B=AQ$，所以由\cref{prop:OrthogonalUnitaryMatrix}(1)可得：
	\begin{equation*}
		A=P
		\begin{pmatrix}
			\varLambda & \mathbf{0} \\
...
\begin{note}
	...
	\end{equation*}
	其中$\seq{p}{m}$与$\seq{q}{n}$分别为$P$与$Q$的列向量，则上述奇异值分解等价于如下的秩一分解：
	\begin{equation*}
		A=\sum_{i=1}^r\lambda_ip_iq_i^H
	\end{equation*}
