...
\begin{proof}
	...
	\end{pmatrix}
	\] 
	其中$l=(l_{21},l_{31},\dots,l_{n1})^T$。则由上述构造和归纳假设可验证：
	\begin{align*}
...
\begin{method}
	...
	\begin{align*}
		...
		=&\frac{2}{3}n^3+\operatorname{O}(n^2)
	\end{align*}
\end{method}
\subsubsection{应用}
\begin{method}
...
\begin{theorem}\label{theo:Cholesky}
	设$A\in M_{n}(\mathbb{R}^{})$是对称正定矩阵，则存在唯一一个可逆的下三角矩阵$L$使得$A=LL^T$，其中$L$的对角线元素均为正数。
\end{theorem}
\begin{proof}
	由\cref{theo:PositiveDefinite}(3.6)、\cref{prop:InvertibleMatrix}(3.a)和\cref{theo:LU}可知$A$存在唯一的LU分解$A=LU$。令$U=(u_{ij})$，由\cref{theo:PositiveDefinite}(5)、\cref{prop:Determinant}(11)(12)、\cref{theo:LU}可知存在$D=\operatorname{diag}\{u_{11},u_{22},\dots,u_{nn}\}$的逆矩阵$D^{-1}$。令$\tilde{U}=D^{-1}U$，于是由\cref{prop:Transpose}(4)可得：
	\begin{equation*}
		\tilde{U}^TDL^T=A^T=A=LD\tilde{U}
	\end{equation*}
	由\cref{prop:Determinant}(11)(12)和\cref{prop:InvertibleMatrix}(3.a)可知$\tilde{U}$可逆，于是：
	\begin{equation*}
		L^T\tilde{U}^{-1}=D^{-1}(\tilde{U}^T)^{-1}LD
	\end{equation*}
	根据\cref{prop:InvertibleMatrix}(18)和\cref{prop:MatrixMultiplication}(6)可知上式左侧为一个单位上三角矩阵，上式右侧为一个下三角矩阵，所以有：
	\begin{equation*}
		L^T\tilde{U}^{-1}=D^{-1}(\tilde{U}^T)^{-1}LD=I_n
	\end{equation*}
	于是$L^T=\tilde{U}$，即：
	\begin{equation*}
		A=LDL^T
	\end{equation*}
	因为$A$正定，所以$D$主对角线上的元素都为正数。令$\tilde{L}=L\operatorname{diag}\{\sqrt{u_{11}},\sqrt{u_{22}},\dots,\sqrt{u_{nn}}\}$，则$A=\tilde{L}\tilde{L}^T$。由\cref{prop:InvertibleMatrix}(3.a)可知$\tilde{L}$可逆。
\end{proof}
\begin{method}
	为求$L=(l_{ij})$使得$A=(a_{ij})=LL^T$，将等式两边展开：
	\begin{equation*}
	...
	\begin{align*}
		...
		=&\frac{1}{3}n^3+\operatorname{O}(n^2)
	\end{align*}
...
\begin{proof}
	...
	\textbf{(2)唯一性：}如果$A=QR=Q'R'$是两种QR分解，其中$Q,Q'$都是正交矩阵，$R,R'$都是上三角矩阵。由\cref{prop:OrthogonalUnitaryMatrix}(1)可得$Q'^{-1}=Q'^T$，根据$R$的对角元素都为正数、\cref{prop:Determinant}(12)和\cref{prop:InvertibleMatrix}(3.a)可得$Q'^TQ=R'R^{-1}$。由\cref{prop:OrthogonalUnitaryMatrix}(4)可知$Q'^TQ$是正交矩阵，根据\cref{prop:InvertibleMatrix}(18)和\cref{prop:MatrixMultiplication}(6)可知$R'R^{-1}$是上三角矩阵，由\cref{prop:OrthogonalUnitaryMatrix}(6)可知$Q'^TQ$为对角矩阵且主对角线元素模为$1$，也即存在一个对角矩阵$D=(\pm1,\pm1,\dots,\pm1)$使得$R'R^{-1}=D$，所以$R'=DR$。当$R$的对角元素均为正值时，上述$D$只能取单位矩阵，因此$Q'=Q,\;R'=R$，唯一性得证。
\end{proof}
\begin{note}
