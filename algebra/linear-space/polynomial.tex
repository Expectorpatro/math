...
\begin{proof}
	...
	\begin{equation*}
		f(x)=h_2(x)h_1(x)f(x)
	\end{equation*}\par
	当$f(x)=0$时，$g(x)=0$，满足条件。当$f(x)\ne0$时，由\cref{prop:UnivariatePolynomialRing}(3)可得$h_2(x)h_1(x)=1$，根据\cref{prop:UnivariatePolynomialRing}(1)可知$\deg h_1(x)=\deg h_2(x)=0$，于是$h_1(x),h_2(x)\in K\backslash\{0\}$，令$a=h_2(x)$即可。\par
