...
\subsection{平方根阵}
\begin{definition}
	设对称阵$A\in M_n(\mathbb{R})$，其特征值记为$\lambda_i,\;i=1,2,\dots,n$。因为$A$是一个实对称阵，由\cref{prop:HermitianMatEigen}(3)可知存在正交矩阵$Q$使得$A=Q^{\top}\operatorname{diag}\{\seq{\lambda}{n}\}Q$。若$A\geqslant0$，由\cref{theo:PositiveSemidefinite}(3.5)可知$\lambda_i\geqslant0,\;i=1,2,\dots,n$，记：
	\begin{equation*}
...
\begin{proof}
	...
	\end{equation*}
	即$\lambda_i^{\frac{1}{2}}$为$A^{\frac{1}{2}}$的特征值，而$\lambda_i\geqslant0$，由\cref{theo:PositiveSemidefinite}(3.5)可知$A^{\frac{1}{2}}\geqslant0$。\par
	(3)显然：
	\begin{equation*}
