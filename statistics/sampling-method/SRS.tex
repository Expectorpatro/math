...
\begin{proof}
	...
	\end{equation*}
	最后一式由SRS的定义即可得到。\par
	(2)由(1)、\cref{prop:MeasurableIntegral}(6)、(3)（先看下面$\mu$的估计）和\cref{prop:Variance}(1)可得：
	\begin{align*}
	...
	\end{align*}\par
	(3)由(1)和\cref{prop:MeasurableIntegral}(6)可得：
	\begin{equation*}
	...
	\end{align*}
	由(2)和\cref{prop:MeasurableIntegral}(6)可知$\widehat{\operatorname{Var}}(\hat{\mu})=\left(1-\dfrac{n}{N}\right)\dfrac{\hat{\sigma}^2}{n}$是$\operatorname{Var}(\hat{\mu})$的无偏估计。由\cref{theo:CLT}可知：
	\begin{equation*}
	...
	\begin{equation*}
		\hat{\mu}\pm u_{1-\frac{\alpha}{2}}\times\sqrt{\widehat{\operatorname{Var}}(\hat{\mu})}
	\end{equation*}\par
	(4)由(3)和\cref{prop:MeasurableIntegral}(6)立即可得。
\end{proof}
\begin{note}
...
\begin{proof} 
	...
	(3)由(2)和\cref{prop:MeasurableIntegral}(6)得到。
\end{proof}
