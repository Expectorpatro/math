...
\end{theorem}
\begin{proof}
	因为$\{x_m^{(1)}\},\{x_m^{(2)}\},\dots,\{x_m^{(n)}\}$线性无关，由\cref{theo:CasoratiEq0LinearDependent}可知存在$m\in\mathbb{Z}^{}$使得Casorati行列式$\det C(m)\ne0$，于是此时的Casorati矩阵$C(n)$可逆。任取$L(x_m)=0$的一个解$\{y_m\}$，则$y_m,\mathcal{B}y_m,\dots,\mathcal{B}^{n-1}y_m$是确定的数，于是关于$(\seq{c}{n})^{\top}$的线性方程组：
	\begin{equation*}
		\begin{pmatrix}
		...
		\end{pmatrix}
	\end{equation*}
	存在唯一解$(\seq{c^{\star}}{n})^{\top}$，即在这$n$个位置处$\{y_m\}$可以由$\{x_m^{(1)}\},\{x_m^{(2)}\},\dots,\{x_m^{(n)}\}$线性表出。令：
	\begin{equation*}
		z_m=c_1^{\star}x_m^{(1)}+c_2^{\star}x_m^{(2)}+\cdots+c_n^{\star}x_m^{(n)},\;\forall\;m\in\mathbb{Z}^{}
	\end{equation*}
