...
\begin{derivation}
	...
	\end{equation*}
	由\cref{theo:StationarySeriesAutoCovariancePE}(3)可知以$\dfrac{1}{n}$为分母构成的自协方差矩阵$\hat{\Gamma}_n$是正定的，根据\cref{theo:PositiveSemidefinite}(6)$\hat{\Gamma}_n$可逆，所以有$\alpha=\hat{\Gamma}_n^{-1}[\hat{\gamma}(n+k-1),\hat{\gamma}(n+k-2),\dots,\hat{\gamma}(k)]^T$，最佳线性预测的预测值即为：
	\begin{equation*}
