...
\begin{proof}
	...
	\begin{equation*}
		|X_t-Y_t|=\left|\sum_{i=1}^{s}\sum_{j=0}^{r_i-1}c_{ij}t^{j}z_i^{-t}\right|\leqslant \operatorname{O}\left[\left(\min_i\{|z_i|\}\right)^{-t}\right]\qedhere
	\end{equation*}
\end{proof}
\begin{note}
...
\begin{property}\label{prop:ARp}
	...
	\begin{enumerate}
		...
		\begin{gather*}
			...
			\gamma(0)=\boldsymbol{\gamma}_n^{\top}\alpha+\sigma^2,\quad n\geqslant p \\
			\boldsymbol{\gamma}_n=
			\begin{pmatrix}
				\gamma(1) \\
				\gamma(2) \\
...
\begin{proof}
	...
	\begin{align*}
		...
		&=\alpha^{\top}\Gamma_n\alpha+\sigma^2=\alpha^{\top}\boldsymbol{\gamma}_n+\sigma^2,\quad n\geqslant p
	\end{align*}\par
...
\begin{property}
	...
	\begin{enumerate}
		...
		\end{gather*}
		记上述矩阵为$\Gamma_{p,q}$；
		\item $\{X_t\}$的自协方差函数与自相关函数具有截尾性\info{应该是拖尾，再看看}，即对任意的$k>q$有$\gamma(k)=\rho(k)=0$，且二者的模随着$n$的增大指数衰减；
		\item $\{X_t\}$具有谱密度：
		\begin{equation*}
...
\begin{proof}
	...
	\end{align*}\par
	(4)截尾性由(3)立即得出。由\cref{ineq:CauchySchiwarz}、(2)和\cref{theo:ARMAPQSolution}(1)中的$|\psi_i|=o(\rho^{-1})$可得：
	\begin{align*}
