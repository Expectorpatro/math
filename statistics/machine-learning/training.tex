...
\begin{equation*}
	\theta^\ast\in\operatorname*{arg\,min}_{\theta\in\Theta}R(\theta)
\end{equation*}
...
\begin{equation*}
	\hat{\theta}\in\operatorname*{arg\,min}_{\theta\in\Theta}\hat{R}(\theta)
\end{equation*}
...
\end{equation*}
最小化$\hat{R}(\theta)$与最大化$Q_{\mathcal{D}}(\theta)$等价。
%\begin{definition}
...
\begin{definition}
	...
	\begin{equation*}
		\hat{f}_c\in\mathop{\arg\min}_{f\in\mathcal{F}_c}\hat{R}_{\mathrm{train}}(f)
	\end{equation*}
	称为约束形式的\gls{Regularization}。其中，$c$称为复杂度上界。
\end{definition}
实际计算中更常使用正则化的惩罚形式：
\begin{equation*}
	\hat{f}_\lambda\in\mathop{\arg\min}_{f\in\mathcal{F}}\left\{\hat{R}_{\mathrm{train}}(f)+\lambda\Omega(f)\right\},\;\lambda\geqslant0
\end{equation*}
