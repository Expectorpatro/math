...
\begin{proof}
	(1)由信息熵的定义立即可得。\par
	(2)由于$\log(x)$在$(0,+\infty)$上为凹函数\info{补充判断理由}，根据\cref{ineq:Jensen}可得：
	\begin{align*}
...
\begin{proof}
	...
	\end{equation*}
	并且$\log(x)$在$(0,+\infty)$上为凹函数\info{补充判断理由}，由\cref{ineq:Jensen}可知：
	\begin{equation*}
...
\subsection{决策树}
决策树从根节点开始，根据某个条件将样本递归地分到不同子节点。内部节点保存判断规则，叶节点保存最终预测。下图以判断今天是否适合户外活动为例；方括号中的两个数字依次表示适合和不适合的历史样本数。对一个新样本，只需从根节点开始依次判断，直至到达叶节点。
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[
		>=Stealth,
		...
		edge/.style={draw=gray!75, very thick, ->},
	]
		\node[decision] (root) at (0,0) {今天下雨吗？\\样本数：$100$，类别计数：$[60,40]$};
		\node[leafGood] (dry) at (-4,-2.35) {预测：适合户外活动\\样本数：$60$，$[52,8]$};
		\node[decision] (wind) at (2.7,-2.35) {风力较大吗？\\样本数：$40$，类别计数：$[8,32]$};
		\node[leafBad] (windy) at (0.7,-4.7) {预测：不适合户外活动\\样本数：$28$，$[1,27]$};
		\node[leafGood] (calm) at (4.7,-4.7) {预测：适合户外活动\\样本数：$12$，$[7,5]$};
		\draw[edge] (root) -- node[above left, font=\small] {否} (dry);
		\draw[edge] (root) -- node[above right, font=\small] {是} (wind);
		\draw[edge] (wind) -- node[above left, font=\small] {是} (windy);
		\draw[edge] (wind) -- node[above right, font=\small] {否} (calm);
	\end{tikzpicture}
	\caption{二分类决策树示意图。每次判断都把当前节点中的样本分到两个子节点，叶节点以占比最高的类别作为预测。}
