...
\end{equation*}
这里，$\widehat{\mathcal{R}}_{\mathrm{train}}$为观测数据集上的经验风险，只度量模型在观测数据集上的平均损失。需要注意，我们最终关心的通常不是模型在观测数据集上的表现，而是模型对同一分布中新样本的预测能力，即总体风险：
\begin{equation*}
	\mathcal{R}(\theta)\coloneq\operatorname{E}\{L[Y,f_\theta(X)]\}
\end{equation*}\par
有时持续降低$\widehat{\mathcal{R}}_{\mathrm{train}}$未必会同时降低$\mathcal{R}$：模型可能不是在提取能够推广到新样本的稳定规律，而是利用观测数据中的偶然噪声、异常点甚至样本的细微特征，对有限训练集作近似记忆。此时训练损失较低，而新样本上的损失较高，这种现象称为\gls{Overfitting}。\par
\begin{figure}[htbp]
	\centering
	\makebox[\textwidth][c]{%
		\begin{tikzpicture}
...
\end{figure}
从\cref{fig:overfitting-three-points}可以看出，三条复杂程度不同的曲线都完美地拟合了由三个观测点构成的观测数据集，因此它们在该观测集上取得了相同且最小的经验风险$\widehat{\mathcal{R}}_{\mathrm{train}}(\theta)$。然而，在训练样本之外，若新的观测仍来自真实关系$y=x$，这些模型会产生较大的预测误差，即它们存在过拟合的现象。由此可见，仅凭观测数据上的经验风险无法判断模型的真实预测能力；即使多个模型具有相同的训练误差，它们的总体风险$\mathcal{R}(\theta)$也可能存在显著差异。\par
综上，我们需要关注差值：
\begin{equation*}
