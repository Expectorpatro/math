...
\subsection{模型学习}
神经网络通常以经验风险$\hat{R}(\theta)$为训练目标，并利用\cref{method:GradientDescent}反复更新参数。由于训练集往往包含大量样本，下面根据每次用于计算梯度的样本数量，介绍梯度下降在神经网络训练中的三种实现形式。以下用$\eta>0$表示学习率，用$g_t$表示第$t$次迭代中使用的梯度或梯度估计。
\begin{definition}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		\Require Initial parameter $\theta_0$, learning rate $\eta$, number of iterations $T$, training set $\mathcal{D}$
		\Ensure Final parameter estimate $\theta_T$
		\For{$t = 1,2,\dots,T$}
			\State Compute the empirical risk $\hat{R}(\theta_t)$ on the whole dataset $\mathcal{D}$
			\State Compute the full gradient $g_t=\nabla\hat{R}(\theta_t)$
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		\Require Initial parameter $\theta_0$, learning rate $\eta$, number of iterations $T$, training set $\mathcal{D}=\{(x_i,y_i)\}_{i=1}^n$
		\Ensure Final parameter estimate $\theta_T$
		\For{$t = 1,2,\dots,T$}
			\State Randomly sample one data point $(x_{i_t}, y_{i_t})$ from $\mathcal{D}$
			\State Compute the stochastic gradient
			\[
				g_t = \left.\nabla_\theta L[y_{i_t},f_\theta(x_{i_t})]\right|_{\theta=\theta_t}
			\]
			\State Update the parameter:
			\[
				\theta_{t+1} \gets \theta_t - \eta\, g_t
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		\Require Initial parameter $\theta_0$, learning rate $\eta$, number of iterations $T$, training set $\mathcal{D}$, batch size $B$
		\Ensure Final parameter estimate $\theta_T$
		\For{$t = 1,2,\dots,T$}
			...
			\[
				...
				\left.\nabla_\theta L[y_i,f_\theta(x_i)]\right|_{\theta=\theta_t}
			\]
			\State Update the parameter:
			\[
				\theta_{t+1} \gets \theta_t - \eta\, g_t
