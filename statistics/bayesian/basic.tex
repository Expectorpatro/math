...
\begin{proof}
	...
	\begin{align*}
		p(\tilde{y}|y)
		&=\int p(\tilde{y}|\mu,y)p(\mu|y)\dif\mu=\int p(\tilde{y}|\mu)p(\mu|y)\dif\mu \\
		&\propto\int\exp\left[-\frac{(\tilde{y}-\mu)^2}{2\sigma^2}\right]\exp\left[-\frac{(\mu-\mu_1)^2}{2\sigma_1^2}\right]\dif\mu	
	\end{align*}
