...
%	\begin{gather*}
%		L(\eta,\mathbf{X})=\prod_{i=1}^{m}C(\eta)\exp\left[\eta^TT(X_i)\right]h(X_i) \\
%		\ell(\eta,\mathbf{X})=\sum_{i=1}^{m}\ln\{C(\eta)\exp[\eta^TT(X_i)]h(X_i)\}=m\ln C(\eta)+\sum_{i=1}^{m}\eta^TT(X_i)+\sum_{i=1}^{m}\ln h(X_i) \\
