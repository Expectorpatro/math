...
\begin{theorem}[Rao-Blackwell Theorem]
	...
	设$(X,\mathscr{A},\mathscr{P})$是参数结构，$\Theta$是参数空间，$\mathbf{X}$为从总体$F$中抽取的简单样本，$g(\theta)$为待估量，$\delta(\mathbf{X})$是$g(\theta)$的估计量，$T$是$\mathscr{P}$的充分统计量，损失函数$L[g(\theta),d]$是关于$d$的凸函数。若$\delta(\mathbf{X}),L[g(\theta),\delta(\mathbf{X})]\in L_1(X)$，则：
	\begin{equation*}
	...
	\end{equation*}
	若$L[g(\theta),d]$关于$d$严格凸，则等号成立当且仅当$\delta(\mathbf{X})=\operatorname{E}_{\theta}[\delta(\mathbf{X})|T]=h(T)\;$a.s.于$(X,\sigma(T),P)$对任意的$P\in\mathscr{P}$成立。若$\delta(\mathbf{X})$是$g(\theta)$的无偏估计量，则$h(T)$也是$g(\theta)$的无偏估计量。
\end{theorem}
\begin{proof}
	因为$T$是充分统计量，所以对任意的$P\in\mathscr{P}$有$P(\mathbf{X}|T)$与$\theta$无关，于是$h(T)=\operatorname{E}_{\theta}[\delta(\mathbf{X})|T]$也与$\theta$无关，由条件期望的定义可得$h(T)$可测，于是$h(T)$是一个统计量。因为$\delta(\mathbf{X})\in L_1(X)$，由\cref{prop:ConditionalExpectation}(3)可知$h(T)\in L_1(X)$。\par
	...
	\end{equation*}
	当$L[g(\theta),d]$关于$d$严格凸时等号成立a.s.于$(X,\sigma(T),P)$对任意的$P\in\mathscr{P}$成立当且仅当$\delta(\mathbf{X})=\operatorname{E}_{\theta}[\delta(\mathbf{X})|T]=h(T)\;$a.s.于$(X,\sigma(T),P)$对任意的$P\in\mathscr{P}$成立。根据\cref{prop:MeasurableIntegral}(7)和\cref{prop:ConditionalExpectation}(3)，对上式两边同时求期望可得：
	\begin{align*}
	...
	\end{align*}\par
	若$L[g(\theta),d]$关于$d$严格凸，由\cref{prop:MeasurableIntegral}(8)可知当$\delta(\mathbf{X})=\operatorname{E}_{\theta}[\delta(\mathbf{X})|T]=h(T)\;$a.s.于$(X,\sigma(T),P)$对任意的$P\in\mathscr{P}$成立时上式等号成立。当上式等号成立时，因为$L[g(\theta),\delta(\mathbf{X})]\in L_1$，由\cref{prop:MeasurableIntegral}(6)可得：
	\begin{equation*}
...
\begin{corollary}\label{cor:Rao-Blackwell}
	若损失函数$L[g(\theta),d]$是关于$d$的严格凸函数，则UMRUE是充分统计量$T$的函数a.s.于$(X,\sigma(T),P)$对任意的$P\in\mathscr{P}$成立。
\end{corollary}
\begin{proof}
...
\begin{theorem}[Lehmann-Scheffe Theorem]
	...
	设$(X,\mathscr{A},\mathscr{P})$是参数结构，$\Theta$是参数空间，$\mathbf{X}$为从总体$F$中抽取的简单样本，$g(\theta)\in\mathbb{R}^{}$为U可估的待估量，损失函数$L[g(\theta),d]$是关于$d$的凸函数。若$\mathscr{P}$存在完全充分统计量$S(\mathbf{X})$，则$g(\theta)$的UMRUE存在，任一$g(\theta)$的无偏估计量关于$S(\mathbf{X})$的Rao-Blackwell改进都是UMRUE。若$L[g(\theta),d]$关于$d$严格凸，则$g(\theta)$的UMRUE在a.s.于$(X,\mathscr{F},P)$对任意的$P\in\mathscr{P}$成立的意义下唯一。
\end{theorem}
\begin{proof}
	任取$g(\theta)$的一个无偏估计量$\delta(\mathbf{X})$，由\cref{theo:Rao-Blackwell}可知$T_1=\operatorname{E}_{\theta}[\delta(\mathbf{X})|S(\mathbf{X})]$仍是$g(\theta)$的一个无偏估计量且风险函数值比$\delta(\mathbf{X})$更小。任取$g(\theta)$的另一无偏估计量$\delta'(\mathbf{X})$，同理可知$T_2=\operatorname{E}_{\theta}[\delta'(\mathbf{X})|S(\mathbf{X})]$仍是$g(\theta)$的一个无偏估计量且风险函数值比$\delta'(\mathbf{X})$更小。因为$g(\theta)\in\mathbb{R}^{}$，所以$\delta(\mathbf{X}),\delta'(\mathbf{X})\in L_1(X)$，于是对任意的$\theta\in\Theta$，由\cref{prop:ConditionalExpectation}(5)(3)、\cref{prop:MeasurableIntegral}(6)可得：
	\begin{align*}
...
\begin{proof}
	...
	\end{align*}\par
	\textbf{(1)充分性：}由\cref{prop:UnbiasedEstimator}可知对任意$g(\theta)$的无偏估计$\delta'(\mathbf{X})$，都存在一个$f(\mathbf{X})\in A$使得$\delta'(\mathbf{X})=\delta(\mathbf{X})+f(\mathbf{X})$。由\cref{prop:Variance}(3)和\cref{prop:NonnegativeMeasurableIntegral}(2)可得：
	\begin{align*}
...
\begin{proof}
	...
	\end{align*}
	于是对任意的零无偏估计$f$，根据\cref{prop:MeasurableIntegral}(6)可得：
	\begin{align*}
	...
	\end{align*}
	由\cref{ineq:cauchy-schiwarz-expectations}\info{验证$\varphi\in L_2$}和\cref{prop:MeasurableIntegral}(6)可得：
	\begin{align*}
...
\begin{note}
	...
	\end{equation*}
	由\cref{prop:FrechetDifferential}(5)可知也即：
	\begin{equation*}
...
\begin{definition}
	...
	则称：
	\begin{equation*}
		I(\theta)=\operatorname{Cov}_{\theta}[S_{\theta}(x)]=\operatorname{E}_{\theta}[S_{\theta}(x)S_{\theta}^{\top}(x)]
	\end{equation*}
...
\begin{property}\label{prop:FIM}
	...
	\begin{enumerate}
		...
		\end{equation*}
		\item Fisher信息量与参数选择有关，若$\theta=h(\xi)$且$h$具有所有一阶偏导数，则$I(\xi)=J_{\theta}^{\top}(\xi)I(\theta)J_{\theta}(\xi)$，其中$J_{\theta}(\xi)$是$\theta$关于$\xi$的Jacobian矩阵；
		...
		\item 若$\{p_{\theta}(x):\theta\in\Theta\}$是满秩指数族的概率函数族，$T(x)$的每一个分量都在$L_2(X)$中，令$\theta=\operatorname{E}_{\eta}(T)$，则$I(\eta)=\operatorname{Cov}_{\eta}(T),\;I(\theta)=[\operatorname{Cov}_{\eta}^{\top}(T)]^{-1}$；
	\end{enumerate}
\end{property}
\begin{proof}
	...
	(2)由正则条件和\cref{prop:MeasurableIntegral}(6)可得：
	\begin{gather*}
	...
	\begin{align*}
		...
		&=\operatorname{E}_{\xi}\left[\left(\frac{\partial\theta}{\partial\xi_i}\right)^{\top}S_{\theta}(x)S_{\theta}^{\top}(x)\left(\frac{\partial\theta}{\partial\xi_j}\right)\right]
	\end{align*}
	于是结论成立。\par
	...
	所以：
	\begin{equation*}
		I(\theta)_{ij}=\sum_{k=1}^{n}\sum_{l=1}^{n}\operatorname{E}_{\theta}[S_{\theta,k}(x_k)S_{\theta,k}^{\top}(x_k)]
	\end{equation*}
	...
	\end{equation*}\par
	因为$T$是统计量，所以根据\cref{prop:ExponentialFamily}(6)、\cref{prop:MeasurableIntegral}(6)和\cref{prop:CovMat}(6)可得：
	\begin{align*}
	...
	\end{equation*}
	因为指数族满秩，由\cref{prop:ExponentialFamily}(3)可知$T(x)$的分量线性无关，所以$\operatorname{Cov}_{\eta}(T)$可逆\info{需要严格证明}，根据\cref{prop:InvertibleMatrix}(12)即可得出结论。
\end{proof}
...
\begin{proof}
	...
	\end{equation*}
	根据\cref{prop:MeasurableIntegral}(6)和条件可得：
	\begin{align*}
