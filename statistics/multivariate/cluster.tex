...
\begin{equation*}
	\mu_k=\frac{1}{n_k}\sum_{i\in C_k}x_i,\quad\bar{x}=\frac{1}{n}\sum_{i=1}^n x_i
\end{equation*}
\subsubsection{内部指标}
\begin{definition}
...
\begin{algorithm}[H]
	...
	\label{alg:Kmodes}
	\begin{algorithmic}[1]
		\Require Categorical data $\mathcal{D}=\{x_i\}_{i=1}^n$, where $x_i=(x_{i1},\dots,x_{ip})$;
		number of clusters $K$;
		maximum number of iterations $T$
		\Ensure Cluster assignments $\{C_1,\dots,C_K\}$;
		modes $\{m_1,\dots,m_K\}$
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\State Initialize numerical prototypes $\mu_1,\dots,\mu_K$ \Comment{Initialization step}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\Ensure Cluster assignments $\{C_1,\dots,C_{K}\}$;
...
然而，从概率分布的角度看，这两个类别具有明确的差异。对于同一个观测值$x$，可以分别考察它由两个正态分布产生的可能性。例如，当$|x|$较小时，方差较小的$N(0,0.5^2)$通常能够给出更高的密度；而当$|x|$较大时，方差较大的$N(0,1.5^2)$则相对更加可能产生这样的观测。也就是说，即使两个类别具有相同的中心，只要它们的概率分布不同，仍然可能根据各分布对样本的解释能力对潜在类别进行推断。\par
这提示我们可以不再直接将一个簇理解为围绕某个中心形成的点集，或者某个密度连通区域，而是假设每个簇对应一个概率分布。样本首先从某个潜在类别中产生，再由该类别对应的概率分布生成。
\begin{definition}
	设随机向量$X\in\mathbb{R}^p$，隐变量$Z\in\{1,2,\dots,K\}$满足：
	\begin{equation*}
		P(Z=k)=\pi_k,\quad\pi_k>0,\quad\sum_{k=1}^{K}\pi_k=1
	\end{equation*}
	并且在给定$Z=k$时有：
	\begin{equation*}
		X\mid Z=k\sim N_p(\mu_k,\Sigma_k)
	\end{equation*}
	其中$\mu_k\in\mathbb{R}^p$，$\Sigma_k\in M_p(\mathbb{R})$为正定矩阵。由\cref{prop:ConditionalProbabilityDensity}(2)可知$X$的密度为：
	\begin{equation*}
		p(x)=\sum_{k=1}^{K}\pi_k\phi_p(x;\mu_k,\Sigma_k)
	\end{equation*}
	其中$\phi_p(x;\mu_k,\Sigma_k)$为$p$维正态分布$N_p(\mu_k,\Sigma_k)$的密度函数。称该模型为具有$K$个成分的\gls{GaussianMixtureModel}，称$\pi_k$为第$k$个混合权重，称$N_p(\mu_k,\Sigma_k)$为第$k$个混合成分。
\end{definition}