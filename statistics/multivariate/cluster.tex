...
\end{enumerate}\par
需要强调的是，外部评价指标的引入并不意味着聚类任务转化为监督学习问题。外部指标的核心作用在于：当研究者在特定数据集中事先掌握某种参考划分时，用以分析既有标签与数据内在结构之间的一致性关系。从而对聚类算法、距离度量或参数设置进行事后评估与比较。因此，外部指标并不参与聚类过程本身，也不作为聚类目标函数进行优化，而是作为一种方法学意义上的对照标准，为研究者理解聚类结果与现实语义之间的关系提供了一种量化参照，帮助研究者理解数据自身在多大程度上支持、修正或挑战已有的分类体系。\par
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\[
		\mathbb{P}(x_i) = \frac{D_i^2}{\sum\limits_{j=1}^n D_j^2}
		\]
		\EndFor
		...
		\State $y_i \gets \arg\min\limits_{k=1,\dots,K} \|x_i-\mu_k\|^2$
		\State Assign $x_i$ to cluster $C_{y_i}$
		\EndFor
		...
		\State $\mu_k \gets \mu_k^{\text{new}} \quad \forall\;k=1,\dots,K$
		\EndFor
	\end{algorithmic}
\end{algorithm}
...
\end{equation*}\par
算法首先对簇中心进行初始化。为缓解随机初始化可能导致的收敛不稳定问题，采用了k-means++初始化策略。具体而言，第一个中心$\mu_1$从样本集合$X$中均匀随机选取。随后，在已经选定前$k-1$个中心的条件下，对每个样本 $x_i$ 计算其到最近已选中心的平方距离：
\begin{equation*}
	D_i^2=\min_{1\leqslant j < k}\|x_i-\mu_j\|^2
\end{equation*}
该距离刻画了样本$x_i$在当前中心集合下对整体目标函数$\Phi$的贡献大小。接下来，根据经验概率分布：
\begin{equation*}
	\mathbb{P}(x_i)=\frac{D_i^2}{\sum_{j=1}^nD_j^2}
\end{equation*}
从样本集中随机选择下一个中心$\mu_k$。该初始化策略倾向于优先选择远离已有中心的样本，从而使初始簇中心在空间中更加分散，为后续迭代提供更有利的起点。\par
在给定当前簇中心$\{\mu_1,\dots,\mu_K\}$的条件下，算法对每一个样本$x_i$计算其到各个簇中心的欧氏距离平方，并将其分配到最近的簇：
\begin{equation*}
	y_i=\arg\min_{k=1,\dots,K} \|x_i-\mu_k\|^2
\end{equation*}
该步骤在固定簇中心的情况下，使目标函数$\Phi$关于簇划分$\{C_k\}$达到最小。\par
在样本分配结果固定后，算法对每一个簇重新计算其中心：
\begin{equation*}
	\mu_k^{\text{new}}=\frac{1}{|C_k|}\sum_{x_i\in C_k}x_i
\end{equation*}\par
样本分配与中心更新两个步骤交替进行，最多执行$T$次迭代。当相邻两次迭代中所有簇中心的变化满足：
\begin{equation*}
	\max_k \|\mu_k^{\text{new}}-\mu_k\|<\varepsilon
\end{equation*}
时，算法提前终止并输出最终的聚类结果。\par
由于每一次分配或更新步骤都会使目标函数$\Phi$的值非增，且$\Phi$在下界零处有界，因此K-means算法在有限步内必然收敛到某一局部最优解。然而，由于目标函数是非凸的\info{证明}，算法的最终结果依赖于初始中心的选取，这也是在实际应用中通常采用k-means++初始化而非完全随机初始化的原因。
...
\begin{enumerate}
	...
	Ward 方法并不直接基于样本对距离，而是选择使合并后类内平方和增量最小的两个簇：
	\begin{equation*}
		\Delta(A,B)=\operatorname{SSE}(A\cup B)-\operatorname{SSE}(A)-\operatorname{SSE}(B)
	\end{equation*}
\end{enumerate}
...
从方法论角度看，DBSCAN 可被视为 OPTICS 在固定邻域半径 $\varepsilon$ 下的特例：当在可达距离图中选定单一阈值时，OPTICS 所得到的聚类结果与 DBSCAN 在相同参数设置下是等价的。相比之下，OPTICS 通过保留完整的密度排序信息，能够同时刻画多尺度密度结构，在不同密度簇共存的数据集中具有更强的表达能力。\par
