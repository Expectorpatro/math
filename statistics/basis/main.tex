...
\begin{proof}
	...
	\end{equation*}
	由\cref{prop:NonnegativeMeasurableIntegral}(2)(3)和\cref{theo:MeasurableCountableIntegral}可知$\nu$是$(X,\mathscr{A})$上的测度，并且$\nu\ll\mu$。根据\cref{theo:RadonNikodym}可知$h(x)$是$\nu$关于$\mu$的Radon-Nikodym导数，由\cref{lem:IntChangeOfMeasure}和\cref{ineq:Holder}可得：
	\begin{align*}
		&\int\exp\left\{[\alpha\theta+(1-\alpha)\vartheta]^{\top}T(x)\right\}h(x)\dif\mu \\
		=&\int\exp\left\{[\alpha\theta+(1-\alpha)\vartheta]^{\top}T(x)\right\}\dif\nu \\
		=&\int\exp\left[\alpha\theta^{\top}T(x)\right]\exp\left[(1-\alpha)\vartheta^{\top}T(x)\right]\dif\nu \\
		\leqslant&\left\{\int\exp\left[\theta^{\top}T(x)\right]\dif\nu\right\}^{\alpha}\left\{\int\exp\left[\vartheta^{\top}T(x)\right]\dif\nu\right\}^{1-\alpha}<+\infty
	\end{align*}
