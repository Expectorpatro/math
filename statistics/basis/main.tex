...
\begin{definition}
	...
	\begin{equation*}
		\left(\prod_{i=1}^nX_i,\prod_{i=1}^n\mathscr{A}_i\right,\prod_{i=1}^n\mathscr{P}_i\right)
	\end{equation*}
...
\begin{definition}
	...
	\end{equation*}
	且支撑$\left\{x:\frac{\dif P_{\theta}}{\dif\mu}(x)>0\right\}$不依赖于未知参数$\theta$，则称$\mathscr{P}$是\gls{ExponentialFamily}。其中$\eta(\theta),T(x)\in\mathbb{R}^{n}$，$n$被称为该指数族的维数，$h$是非负Borel函数，$\xi(\theta)$是实值函数。称：
	\begin{equation*}
...
\begin{proof}
	...
	\end{equation*}
	由(5)和\cref{prop:MeasurableIntegral}(6)可得\info{$C(\eta)$的可积性与Borel可测性}：
	\begin{gather*}
