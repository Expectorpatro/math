...
\begin{definition}\label{model:LinearModel}
	...
	\begin{equation*}
		...
		\end{cases}
	\end{equation*}
	其中$\varepsilon:\Omega\to\mathbb{R}^n$为随机误差向量，则称上述模型为\gls{LinearModel}。称$X$为\gls{DesignMatrix}，随机向量$Y$的一次观测记为$y\in\mathbb{R}^n$，称$y$为\gls{ObservationVector}。
\end{definition}
...
由于随机误差的存在，观测向量$y$一般并不恰好属于$X$的象$\operatorname{Im}(X)$，因此方程$X\beta=y$通常没有解。为此，我们不再要求拟合值$X\beta$与观测向量$y$完全相等，而是要求二者之间的差值$y-X\beta$尽可能接近$\mathbf{0}$。最常用的度量方式是标准范数的平方，由此得到最小二乘法。
\begin{definition}
...
\end{proof}
\begin{derivation}
	由于对任意$b\in\mathbb{R}^p$，向量$Xb$都属于$X$的列空间$\operatorname{Im}(X)$，故所有可能的拟合值为集合：
	\begin{equation*}
		\{Xb:b\in\mathbb{R}^p\}=\operatorname{Im}(X)
	\end{equation*}
	因此，最小二乘问题等价于在子空间$\operatorname{Im}(X)$（\cref{prop:LinearMapping}(6)）中寻找与$y$距离最近的向量，即：
	\begin{equation*}
		\min_{z\in\operatorname{Im}(X)}||y-z||^2
	\end{equation*}
	若$\hat{\beta}$是最小二乘解，并记$\hat{y}=X\hat{\beta}$，则$\hat{y}\in\operatorname{Im}(X)$且满足：
	\begin{equation*}
		||y-\hat{y}||=\min_{z\in\operatorname{Im}(X)}||y-z||
	\end{equation*}
	由正则方程：
	\begin{equation*}
	...
	\end{equation*}
	这表明$y-\hat{y}$与$X$的每一列都正交，从而与整个子空间$\operatorname{Im}(X)$正交。故$\hat{y}=X\hat{\beta}$正是$y$在$\operatorname{Im}(X)$上的正交投影。
\end{derivation}
\begin{figure}[htbp]
	\centering
	\begin{tikzpicture}[
	...
	\end{tikzpicture}
	\caption{$\hat{y}=X\hat{\beta}$是$y$在$\operatorname{Im}(X)$上的正交投影}
\end{figure}
\begin{note}
...
\end{definition}
\begin{note}
	由\cref{theo:LSELinearModel}可知$\beta$的最小二乘估计一般并不唯一，因此不能直接讨论$\beta$本身的估计。由于线性模型通过$\operatorname{E}(Y)=X\beta$刻画参数与观测变量之间的线性关系，我们转而关注$\beta$的线性函数$c^{\top}\beta$。由于实际能够观测到的只有$Y$，自然要考察是否存在$\alpha\in\mathbb{R}^{n}$使得$\alpha^{\top}Y$是$c^{\top}\beta$的线性无偏估计量。
\end{note}
\begin{property}\label{prop:EstimableFunction}
	对于\cref{model:LinearModel}，设$c^{\top}\beta$和$d^{\top}\beta$是可估函数，$\hat{\beta}$是$\beta$的最小二乘估计，则：
	\begin{enumerate}
		...
		\item (Gauss-Markov)当$\sigma^2>0$时，在方差最小的意义下$c^{\top}\hat{\beta}$是$c^{\top}\beta$唯一的BLUE；
		\item 设$\varphi_i=c_i^{\top}\beta,\;i=1,2,\dots,k$都是可估函数，$\seq{\alpha}{k}\in\mathbb{R}$，则$\varphi=\sum\limits_{i=1}^{k}\alpha_i\varphi_i$也是可估的，$\hat{\varphi}=\sum\limits_{i=1}^{k}\alpha_ic_i^{\top}\hat{\beta}$是$\varphi$的无偏估计。当当$\sigma^2>0$时，在方差最小的意义下$\hat{\varphi}$时$\varphi$唯一的BLUE。
	\end{enumerate}
\end{property}
\begin{proof}
	...
	\begin{align*}
		&\operatorname{Cov}(c^{\top}\hat{\beta},d^{\top}\hat{\beta})=\operatorname{Cov}[c^{\top}(X^{\top}X)^-X^{\top}Y,d^{\top}(X^{\top}X)^-X^{\top}Y] \\
	...
	\end{align*}
	根据\cref{prop:Transpose}(4)和\cref{theo:VectorSpaceAAAT}可知$c^{\top}=\alpha^{\top}X\in\mathcal{M}(X^{\top})=\mathcal{M}(X^{\top}X),d=X^{\top}\gamma\in\mathcal{M}(X^{\top})=\mathcal{M}(X^{\top}X)$，由\cref{prop:A-}(3)可得$\operatorname{Cov}(c^{\top}\hat{\beta},d^{\top}\hat{\beta})$与$(X^{\top}X)^-$的选择无关。\par
	...
	于是：
	\begin{equation*}
		\operatorname{Var}(a^{\top}Y)-\operatorname{Var}(c^{\top}\hat{\beta})=\sigma^2||a-X(X^{\top}X)^-c||^2\geqslant0
	\end{equation*}
	所以$\operatorname{Var}(a^{\top}Y)=\operatorname{Var}(c^{\top}\hat{\beta})$当且仅当$a=X(X^{\top}X)^-c$，由\cref{theo:LSELinearModel}可知$a=X(X^{\top}X)^-c\Leftrightarrow a^{\top}=c^{\top}(X^{\top}X)^-X^{\top}\Leftrightarrow a^{\top}y=c^{\top}(X^{\top}X)^-X^{\top}y=c^{\top}\hat{\beta}$。\par
	(7)由(2)可知$\varphi$是可估的。\par
	由(4)可得$c_i^{\top}\hat{\beta}$是$c_i^{\top}\beta$的无偏估计，根据\cref{prop:MeasurableIntegral}(6)可得：
	\begin{equation*}
...
\end{proof}
\begin{definition}
	对于\cref{model:LinearModel}，若$c^{\top}\beta$是可估函数，称$c^{\top}\hat{\beta}$为$c^{\top}\beta$的LSE，其中$\hat{\beta}$为$\beta$的最小二乘估计。
\end{definition}
\subsubsection{残差}
\begin{definition}
	对于\cref{model:LinearModel}，记$\hat{y}=X\hat{\beta}$，其中$\hat{\beta}$是$\beta$的最小二乘估计。称$\hat{e}=Y-\hat{y}$为\gls{ResidualVector}，称$\hat{e}^{\top}\hat{e}$为\gls{SSE}。
\end{definition}
\begin{property}\label{prop:ehat}
	对于\cref{model:LinearModel}，$\hat{\beta}$为$\beta$的最小二乘估计，则残差向量$\hat{e}$满足：
	\begin{enumerate}
...
\begin{lemma}\label{lem:AXTXAT}
	若$A\in M_{k\times p}(K)$，$\operatorname{rank}(A)=k$且$\mathcal{M}(A^{\top})\subseteq\mathcal{M}(X^{\top})$，则$A(X^{\top}X)^{-}A^{\top}$正定，于是根据\cref{theo:PositiveDefinite}(6)可知$A(X^{\top}X)^{-}A^{\top}$可逆。
\end{lemma}
\begin{proof}
	由\cref{theo:VectorSpaceAAAT}可知$\mathcal{M}(A^{\top})\subseteq\mathcal{M}(X^{\top}X)$，于是存在$B\in M_{p\times k}(K)$使得$A^{\top}=X^{\top}XB$。根据\cref{prop:Transpose}(4)可得：
	\begin{equation*}
		A(X^{\top}X)^{-}A^{\top}=B^{\top}X^{\top}X(X^{\top}X)^{-}X^{\top}XB=B^{\top}X^{\top}XB
	\end{equation*}
	注意到对任意的$x\in K^k$有$x^{\top}B^{\top}X^{\top}XBx=||XBx||^2\geqslant0$。若存在非零的$x\in K^k$使得$x^{\top}B^{\top}X^{\top}XBx=||XBx||^2=0$，则：
	\begin{equation*}
