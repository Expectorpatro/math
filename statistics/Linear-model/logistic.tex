...
\begin{definition}\label{model:GLM}
	...
	记
	\[
	\mu_i = \mathbb{E}(y_i \mid x_i) = b'(\theta_i), \qquad
	\operatorname{Var}(y_i \mid x_i) = a(\phi)\, b''(\theta_i),
	\]
	并通过\textbf{链接函数} $g(\cdot)$ 将条件均值与线性预测子联系：
	\begin{equation*}
		g(\mu_i) = \eta_i = x_i^{\top}\beta,
	\end{equation*}
	其中 $x_i$ 为第 $i$ 个观测的 $p\times 1$ 特征向量，$\beta$ 为 $p\times 1$ 的回归系数向量，$\phi$ 为离散参数（dispersion parameter）。
	
	记
	\[
	\mu = (\mu_1,\mu_2,\dots,\mu_n)^{\top}, \quad
...
\begin{definition}\label{model:Logistic}
	...
	\begin{equation*}
		...
		\operatorname{E}(y_i|x_i)=p_i =\operatorname{P}(y_i=1|x_i) =  \frac{1}{1+\exp(-x_i^{\top}\beta)}, \quad i=1,2,\dots,n
	\end{equation*}
	其中 $y_i$相互独立，$x_i$ 为第 $i$ 个观测的 $p\times 1$ 特征向量，$\beta$ 为 $p\times1$ 的回归系数向量。记$p = (p_1,p_2,\dots,p_n)^{\top},\;X=(x_1;x_2;\cdots;x_n),\;y=(\seq{y}{n})$。
\end{definition}
\begin{property}
	...
	\begin{enumerate}
		...
		\begin{equation*}
			\frac{\dif^2\ln L(\beta)}{\dif\beta\dif\beta^{\top}}=-X^{\top}\operatorname{diag}\{p_1(1-p_1),p_2(1-p_2),\dots,p_n(1-p_n)\}X
		\end{equation*}
	\end{enumerate}
\end{property}
\begin{proof}
	...
	\begin{align*}
		...
		&=\sum_{i=1}^{n}(y_i-y_ip_i-p_i+y_ip_i)=\sum_{i=1}^n (y_i - p_i)x_i = X^{\top}(y - p)
	\end{align*}\par
	(4)对上式再求导：
	\begin{align*}
		\frac{\dif^2 \ln L(\beta)}{\dif \beta \dif \beta^{\top}}
