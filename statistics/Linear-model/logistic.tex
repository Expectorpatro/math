...
\begin{equation*}
	X^{\top}WXz=X^{\top}(y-\mu)
\end{equation*}
这其实正是\cref{model:GeneralizedLinearModel}下的正则方程。\par
能不能将这类推广的线性模型统一起来，用一个统一的公式去表达？如果可以的话，能不能尽量扩大这个统一形式的表达范围，使得囊括尽可能多的情况？我们需要来分析产生上述线性方程组的原因。设模型的参数为$\theta$。\par
首先分析$\ln L(\beta)$关于$\beta$的梯度：
\begin{equation*}
	\frac{\dif\ln L(\beta)}{\dif\beta}=\frac{\dif\ln L(\beta)}{\dif\theta}\frac{\dif\theta}{\dif\beta}
\end{equation*}
均值和模型参数之间具有一个函数关系，同时均值和$\beta$之间也有函数关系，$X^{\top}(y-\mu)$中
