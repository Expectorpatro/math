...
\begin{proof}
	...
	由于$\{a_1,\dots,a_{j-1}\}$线性无关可保证$q_j'\neq 0$。然后令$r_{jj}=\|q_j'\|$并归一化$q_j = q_j'/r_{jj}$作为$Q$的第$j$列。如此递推下去，我们得到$Q=[q_1,\dots,q_n]$是一个正交矩阵，而对应的$R=[r_{ij}]$是上三角矩阵，其中$r_{ij}=q_i^T a_j$（当$i\le j$）或$r_{ij}=0$（当$i>j$）。由上述构造可以验证$A=QR$。这种方法即为Gram-Schmidt正交化算法。
\end{proof}
