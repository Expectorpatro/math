...
\section{变分法}
\begin{definition}
	设$\varphi,\mu$是可测空间$(X,\mathscr{F})$上的概率测度，$\varphi\ll\mu$，根据\cref{theo:RandonNikodym}，称：
	\begin{equation*}
		\int_{X}\ln\frac{\dif\varphi}{\dif\mu}\dif\varphi
	\end{equation*}
...
\begin{note}
	...
	设$y\mid x$的概率分布为$\varphi$，其近似概率分布为$\psi$，$\varphi,\psi\ll\mu$，$\varphi,\psi,\mu$是可测空间$(X,\mathscr{F})$上的概率测度，根据\cref{theo:RandonNikodym}，记：
	\begin{equation*}
		q(y)=\frac{\dif\psi}{\dif\mu},\quad p(y\mid x)=\frac{\dif\varphi}{\dif\mu}
	\end{equation*}
	由\cref{prop:RandonNikodym}(5)和\cref{lem:IntChangeOfMeasure}可知：
	\begin{equation*}
	...
	\end{equation*}
	由于KL散度公式中存在未知的$p(y\mid x)$，于是需要将其转变为易于求解的形式。由\info{贝叶斯公式}和\cref{prop:MeasurableIntegral}(6)可得：
	\begin{align*}
...
\subsection{EM算法}
在极大似然估计中，若观测变量$f$为从可控参数结构$(X,\mathscr{A},\mathscr{P})$到可测空间$(Y,\mathscr{B})$上的可测映射，参数空间为$\Theta$，控制测度为$\mu$，根据\cref{theo:RandonNikodym}，记$\dfrac{\dif P_{\theta}}{\dif\mu}(f)=p_{\theta}(f)$，那么目标函数可以写成：
\begin{equation*}
...
但是在引入隐变量之后，我们需要通过对隐变量积分，才能得到观测变量的边际似然。设$g$为概率空间$(X,\mathscr{A},P)$到可测空间$(Z,\mathscr{C})$上的可测映射，$Pg^{-1}\ll\nu$，根据\cref{theo:RandonNikodym}，记$\dfrac{\dif Pg^{-1}}{\dif\nu}=q(g)$，由\info{全概率公式}和\cref{lem:IntChangeOfMeasure}可得：
\begin{equation*}
...
由\cref{ineq:Jensen}、\info{贝叶斯公式}和\cref{prop:MeasurableIntegral}(6)可得：
\begin{align*}
...
记：
\begin{equation*}
	L(q,\theta)=\int_{Z}q(g)\ln p_{\theta}(f,g)\dif\nu-\int_{Z}q(g)\ln q(g)\dif\nu
\end{equation*}
则有:
\begin{equation*}
	L(q,\theta)\leqslant\ln p_{\theta}(f)
\end{equation*}
这个$L(q,\theta)$就是边际对数似然$\ln p_{\theta}(f)$的一个下界。\par
EM算法的基本思想就是：既然直接最大化$\ln p_{\theta}(f)$比较困难，那么可以转而反复提高这个下界。若一组$\hat{q},\hat{\theta}$使得$L(q,\theta)$达到最大，则$\hat{\theta}$也应该是$\theta$的极大似然估计。\par
进一步地，由\cref{prop:RandonNikodym}(5)、\cref{lem:IntChangeOfMeasure}和\info{贝叶斯公式}可知这个下界与真实边际对数似然之间的差距可以精确写成一个KL散度：
\begin{align*}
...
于是：
\begin{equation*}
	\ln p_{\theta}(f)=L(q,\theta)+\operatorname{KL}[q(g)\|p_{\theta}(g\mid f)]
\end{equation*}\par
\begin{method}[Expectation-Maximization Algorithm]
	EM算法可以理解为对$L(q,\theta)$进行坐标上升。具体地说，在第$t$轮迭代中，假设当前参数为$\theta^{(t)}$。EM算法交替更新$q$和$\theta$。\par
	首先固定$\theta=\theta^{(t)}$，最大化$L(q,\theta^{(t)})$。由于
	\begin{equation*}
		\ln p_{\theta^{(t)}}(f)=L(q,\theta^{(t)})+\operatorname{KL}[q(g)\|p_{\theta^{(t)}}(g\mid f)]
	\end{equation*}
	而左边与$q$无关，是一个定值，所以最大化$L(q,\theta^{(t)})$等价于最小化$\operatorname{KL}[q(g)\|p_{\theta^{(t)}}(g\mid f)]$。当$q^{(t+1)}(g)=p_{\theta^{(t)}}(g\mid f)$时，KL散度达到最小值$0$。因此，EM算法的E步就是在当前参数$\theta^{(t)}$下计算隐变量的后验分布。\par
	接着固定$q=q^{(t+1)}$，最大化$L(q^{(t+1)},\theta)$。因为：
	\begin{equation*}
		L[q^{(t+1)},\theta]=\int_{Z}q^{(t+1)}(g)\ln p_{\theta}(f,g)\dif\nu-\int_{Z}q^{(t+1)}(g)\ln q^{(t+1)}(g)\dif\nu
	\end{equation*}
