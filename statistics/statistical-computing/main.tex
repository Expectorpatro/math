...
\begin{method}[Expectation-Maximization Algorithm]
	...
	首先固定$\theta=\theta^{(t)}$，最大化$L(q,y)$。由于
	\begin{equation*}
		\ln p_{\theta^{(t)}}(f)=L(q,y)+\operatorname{KL}[q(g)\|p_{\theta^{(t)}}(g\mid f)]
	\end{equation*}
	而左边与$q$无关，是一个定值，所以最大化$L(q,y)$等价于最小化$\operatorname{KL}[q(g)\|p_{\theta^{(t)}}(g\mid f)]$。当$q^{(t+1)}(g)=p_{\theta^{(t)}}(g\mid f)$时，KL散度达到最小值$0$。因此，EM算法的E步就是在当前参数$\theta^{(t)}$下计算隐变量的后验分布。\par
