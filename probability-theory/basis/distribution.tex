...
\begin{definition}
	...
	\begin{equation*}
		...
		\end{cases}
	\end{equation*}
	其中$a,b\in\mathbb{R}$为参数且$a<b$，则称$X$服从\gls{UniformDistribution}，记作$X\sim\operatorname{U}(a,b)$。
\end{definition}
\begin{densityplot}{uniform}
	\begin{tikzpicture}
		\begin{axis}[
			density plot,
			xmin=-0.4, xmax=1.4,
			ymin=0, ymax=1.25,
			title={$\operatorname{U}(0,1)$},
		]
			\addplot[domain=-0.4:0] {0};
			\addplot[domain=0:1] {1};
			\addplot[domain=1:1.4] {0};
			\addplot[gray, densely dashed] coordinates {(0,0) (0,1)};
			\addplot[gray, densely dashed] coordinates {(1,0) (1,1)};
		\end{axis}
	\end{tikzpicture}
	\caption{均匀分布 $\operatorname{U}(0,1)$ 的概率密度函数}
\end{densityplot}
\begin{property}
	均匀分布族具有如下性质：
	\begin{enumerate}
...
\begin{densityplot}{gamma}
	\begin{tikzpicture}
		\begin{axis}[
			...
			xmin=0, xmax=12,
			ymin=0, ymax=0.3,
			title={$\operatorname{Gamma}(3,1)$},
			]
			\addplot {x^2*exp(-x)/2};
		\end{axis}
	\end{tikzpicture}
	\caption{Gamma分布 $\operatorname{Gamma}(3,1)$ 的概率密度函数，其中 $\lambda$ 为率参数}
\end{densityplot}
\begin{property}
	Gamma分布具有如下性质：
	\begin{enumerate}
...
\begin{densityplot}{beta}
	\begin{tikzpicture}
		\begin{axis}[
			...
			xmin=0, xmax=1,
			ymin=0, ymax=2.6,
			title={$\operatorname{Beta}(2,5)$},
			]
			\addplot {30*x*(1-x)^4};
		\end{axis}
	\end{tikzpicture}
	\caption{Beta分布 $\operatorname{Beta}(2,5)$ 的概率密度函数}
\end{densityplot}
\begin{property}
	Beta分布具有如下性质：
	\begin{enumerate}
...
\begin{densityplot}{normal}
	\begin{tikzpicture}
		\begin{axis}[
			...
			xmin=-4.5, xmax=4.5,
			ymin=0, ymax=0.45,
			title={$\operatorname{N}(0,1)$},
		]
			\addplot {exp(-x^2/2)/sqrt(2*pi)};
		\end{axis}
	\end{tikzpicture}
	\caption{标准正态分布 $\operatorname{N}(0,1)$ 的概率密度函数}
\end{densityplot}
\begin{note}
...
\end{definition}
\begin{note}
	由\cref{prop:MeasurableIntegral}(6)和\cref{prop:CovMat}(3)(4)(5)可得$\operatorname{E}(\mathbf{X})=\boldsymbol{\mu},\;\operatorname{Cov}(\mathbf{X})=AA^{\top}$，所以上定义成立。
\end{note}
...
\begin{definition}
	...
	其概率密度函数可以写为Poisson混合形式：
	\begin{equation*}
		p(y)=e^{-\frac{\lambda}{2}}\sum_{j=0}^{+\infty}\frac{(\lambda/2)^j}{j!}
		\frac{y^{\frac{n}{2}+j-1}e^{-\frac{y}{2}}}{2^{\frac{n}{2}+j}\Gamma\left(\frac{n}{2}+j\right)},\quad y>0
	\end{equation*}
\end{definition}
\begin{densityplot}{chi-square}
	\begin{tikzpicture}
		\begin{axis}[
			density plot,
			domain=0:18,
		...
		\end{axis}
	\end{tikzpicture}
	\caption{中心 $\chi^2$ 分布 $\chi^2_{4,0}$ 的概率密度函数}
\end{densityplot}
...
\begin{definition}
	...
	\begin{equation*}
		...
		\left(1+\frac{x^2}{n}\right)^{-\frac{n+1}{2}},\quad x\in\mathbb{R}
	\end{equation*}
\end{definition}
\begin{densityplot}{student-t}
	\begin{tikzpicture}
		\begin{axis}[
			density plot,
			domain=-5:5,
			xmin=-5, xmax=5,
			ymin=0, ymax=0.42,
			title={$\operatorname{t}_5$},
		]
			\addplot {0.3796066898*(1+x^2/5)^(-3)};
		\end{axis}
	\end{tikzpicture}
	\caption{$t$ 分布 $\operatorname{t}_5$ 的概率密度函数}
\end{densityplot}
\subsubsection{$F$分布}
\begin{definition}
	...
	其概率密度函数为：
	\begin{equation*}
		p(x)=\frac{1}{\operatorname{B}\left(\frac{m}{2},\frac{n}{2}\right)}
		\left(\frac{m}{n}\right)^{\frac{m}{2}}x^{\frac{m}{2}-1}
		\left(1+\frac{m}{n}x\right)^{-\frac{m+n}{2}},\quad x>0
	\end{equation*}
\end{definition}
\begin{densityplot}{f}
	\begin{tikzpicture}
		\begin{axis}[
			...
			xmin=0, xmax=5,
			ymin=0, ymax=0.8,
			title={$\operatorname{F}_{5,10}$},
		]
			\addplot {10.3683675*x^1.5*(1+0.5*x)^(-7.5)};
		\end{axis}
	\end{tikzpicture}
	\caption{$F$ 分布 $\operatorname{F}_{5,10}$ 的概率密度函数}
\end{densityplot}
...
\subsection{随机数的生成}
随机数的生成是对随机问题进行数值检验的重要工具。对于随机现象的分布、概率或数字特征等理论结论，可以按照给定的概率分布生成大量观测值来模拟随机试验，进而检验理论结论是否与数值结果相符。对于含有随机步骤的算法，还可以在相同条件下重复运行，比较多次运行的结果，以考察算法输出的波动及其稳定性。\par
随机数的生成通常以均匀分布为基础。数论中的同余等方法可以帮助我们在计算机上构造具有良好均匀性的伪随机数列，从而实现均匀分布的抽样。但对于概率密度较为复杂的分布，直接抽样往往并不容易。
\subsubsection{逆变换法}
\begin{definition}
...
\begin{note}
	左连续逆实际就是广义的分位点。
\end{note}
若目标分布的分布函数具有容易计算的左连续逆，则可以将均匀分布的观测值直接变换为目标分布的观测值，这种方法称为逆变换法。
\begin{method}
	设$U$是概率空间$(X,\mathscr{F},P)$上服从$\operatorname{U}(0,1)$的随机变量。根据连续型随机变量的定义，在$P$零测集上适当修改$U$可使得$U(X)\subseteq(0,1)$。令：
	\begin{equation*}
		f=F^{\leftarrow}(U)
	\end{equation*}
	逆变换法按如下步骤生成一个观测值：
	\begin{enumerate}
		\item 生成$U$的一个观测值$u\in(0,1)$；
		\item 输出$y=F^{\leftarrow}(u)$。
	\end{enumerate}
	则$f\sim F$，即算法的输出服从分布函数$F$所确定的概率分布。
\end{method}
\begin{proof}
	任取$u\in(0,1)$，由\cref{prop:LeftcontinuousInverse}(1)可知对任意的$y\in\mathbb{R}$有：
	\begin{equation*}
		F^{\leftarrow}(u)\leqslant y\iff u\leqslant F(y)
	\end{equation*}
	于是根据\cref{prop:BorelSigmaField}(1.b)可知：
	\begin{equation*}
		\{u\in(0,1):F^{\leftarrow}(u)\leqslant y\}=(0,F(y)]\cap(0,1)\in\mathcal{B}(\mathbb{R})
	\end{equation*}
	根据\cref{prop:MeasurableFunction}(1.b)可知$F^{\leftarrow}$是Borel函数，由\cref{prop:MeasurableMapping}(2)可知$f=F^{\leftarrow}(U)$是随机变量。对任意的$y\in\mathbb{R}$，由均匀分布的定义可得：
	\begin{equation*}
		P(f\leqslant y)=P(F^{\leftarrow}(U)\leqslant y)=P(U\leqslant F(y))=F(y)
	\end{equation*}
	由$y$的任意性可知$f\sim F$。
\end{proof}
\subsubsection{接受拒绝法}
另一种生成服从给定分布$\pi$的随机数的思想为：可以选取一个容易抽样的概率分布$Q$作为候选分布，先从$Q$中生成候选观测值，再按照适当的概率接受或拒绝该观测值。若接受概率选择得适当，则最终被接受的样本虽然最初来自$Q$，其条件分布却可以恰好等于目标分布$\pi$。这一思想对应的实现就是接受拒绝抽样法。
\begin{method}
	设$(Y,\mathscr{A},\mu)$是$\sigma$有限测度空间，$\tilde{\pi}:Y\to[0,+\infty)$是其上的可测函数，且：
	\begin{equation*}
		Z=\int_{Y}\tilde{\pi}(y)\dif\mu\in(0,+\infty)
	\end{equation*}
	在$(Y,\mathscr{A})$上定义目标概率测度$\pi$：
	\begin{equation*}
		\pi(B)=\frac{1}{Z}\int_{B}\tilde{\pi}(y)\dif\mu,\quad\forall\;B\in\mathscr{A}
	\end{equation*}
	设$Q$是$(Y,\mathscr{A})$上的概率测度，$Q\ll\mu$。根据\cref{theo:RadonNikodym}，取$q=\dfrac{\dif Q}{\dif\mu}$的一个非负版本（不讨论存在性）。若存在$M\in(0,+\infty)$使得：
	\begin{equation*}
		\tilde{\pi}(y)\leqslant Mq(y)\;\text{a.e.于}(Y,\mathscr{A},\mu)
	\end{equation*}
	则令：
	\begin{equation*}
		\alpha(y)=
		\begin{cases}
			\dfrac{\tilde{\pi}(y)}{Mq(y)},&q(y)>0 \\
			0,&q(y)=0
		\end{cases}
	\end{equation*}
	在$\mu$零测集上适当修改$\alpha$，可使$0\leqslant\alpha(y)\leqslant1$对任意的$y\in Y$成立。设$f$为概率空间$(X,\mathscr{F},P)$到$(Y,\mathscr{A})$上的可测映射且$Pf^{-1}=Q$，$U\sim\operatorname{U}(0,1)$且$U$与$f$独立。接受拒绝法按如下步骤生成一个观测值：
	\begin{enumerate}
		\item 从$Q$中抽取候选值，即生成$f$的一个观测值$y\in Y$；
		\item 独立生成$U$的一个观测值$u\in(0,1)$；
		\item 若$u\leqslant\alpha(y)$，则接受并输出$y$；否则拒绝$y$，并与此前试验独立地返回第(1)步。
	\end{enumerate}
	算法一次运行输出最终被接受的观测值$y\in Y$，单次试验的接受概率为$Z/M$，算法以概率$1$在有限步内输出一个值，且算法的输出的概率分布为$\pi$。
\end{method}
\begin{proof}
	对任意的$x\in X$，$f(x)$就是第(1)步生成的候选观测值，$U(x)$就是第(2)步生成的均匀分布观测值。记单次试验的接受事件为：
	\begin{equation*}
		C=\{U\leqslant\alpha(f)\}
	\end{equation*}
	根据\cref{prop:MeasurableFunction}(5.a)(1.b)可知$C\in\mathscr{F}$。\par
	由\cref{prop:Independent}(5.a)可知：
	\begin{equation*}
		P(f,U)^{-1}=Pf^{-1}\times PU^{-1}
	\end{equation*}
	其中$Pf^{-1}$与$PU^{-1}$分别为$f$与$U$的概率分布。任取$A\in\sigma(f)$，则存在$B\in\mathcal{B}(\mathbb{R})$使得：
	\begin{equation*}
		A=\{f\in B\}
	\end{equation*}
	于是由\cref{theo:Fubini}(2)、指示函数的乘积性质、均匀分布的定义、\cref{theo:IntBySubstitution}和\cref{theo:MeasurableCountableIntegral}可得：
	\begin{align*}
		&P(A\cap C)=P\left(\{f\in B\}\cap\{U\leqslant\alpha(f)\}\right) \\
		=&\int_{\mathbb{R}\times[0,1]}I_{B\cap\{U\leqslant\alpha(f)\}}\dif P(f,U)^{-1}=\int_{\mathbb{R}}\int_{[0,1]}I_{B}I_{\{U\leqslant\alpha(f)\}}\dif PU^{-1}\dif Pf^{-1} \\
		=&\int_{\mathbb{R}}I_{B}\left[\int_{[0,1]}I_{\{U\leqslant\alpha(f)\}}\dif PU^{-1}\right]\dif Pf^{-1}=\int_{\mathbb{R}}I_{B}\alpha(f)\dif Pf^{-1} \\
		=&\int_{X}I_{A}\alpha[f(x)]\dif P=\int_{A}\alpha[f(x)]\dif P
	\end{align*}
	根据非负简单函数积分的定义可得：
	\begin{equation*}
		P(A\cap C)=\int_{A}I_C(x)\dif P
	\end{equation*}
	所以对于任意$A\in\sigma(f)$均有：
	\begin{equation*}
		\int_{A}I_C(x)\dif P=\int_{A}\alpha[f(x)]\dif P
	\end{equation*}
	可以验证$\alpha(f)$是$\sigma(f)$可测随机变量，所以根据条件期望的定义可得：
	\begin{equation*}
		\operatorname{E}\left[I_C\mid\sigma(f)\right]=\alpha(f)\;\text{a.e.于$(X,\mathscr{F},P)$}
	\end{equation*}
	即：
	\begin{equation*}
		P(C\mid f)=\alpha(f)\;\text{a.e.于$(X,\mathscr{F},P)$}
	\end{equation*}\par
	任取$B\in\mathscr{A}$。根据\cref{prop:ConditionalProbability}(2)、指示函数的乘积性质、\cref{theo:ConditionalExpectationPullOut}、\cref{prop:MeasurableIntegral}(8)、\cref{theo:MeasurableCountableIntegral}、\cref{lem:IntChangeOfMeasure}和\cref{prop:MeasurableIntegral}(6)可得：
	\begin{align*}
		&P(\{f\in B\}\cap C)=\operatorname{E}[P(\{f\in B\}\cap C\mid f)]=\operatorname{E}\{\operatorname{E}[I_{\{f\in B\}\cap C}\mid f]\} \\
		=&\operatorname{E}\{\operatorname{E}[I_{\{f\in B\}}I_C\mid f]\}=\operatorname{E}\{I_{\{f\in B\}}\operatorname{E}[I_C\mid f]\}=\operatorname{E}[I(f\in B)P(C\mid f)] \\
		=&\operatorname{E}[I(f\in B)\alpha(f)]=\int_{B}\alpha(f)\dif Q=\int_{B}\alpha(f)q(f)\dif\mu=\frac{1}{M}\int_{B}\tilde{\pi}(f)\dif\mu
	\end{align*}
	取$B=Y$可得单次试验的接受概率为：
	\begin{equation*}
		P(C)=\frac{1}{M}\int_{Y}\tilde{\pi}(f)\dif\mu=\frac{Z}{M}>0
	\end{equation*}\par
	若$Q(B)>0$，由\cref{prop:ConditionalProbability}(1)可得：
	\begin{equation*}
		P(\{f\in B\}\mid C)=\frac{P(\{f\in B\}\cap C)}{P(C)}=\frac{\dfrac{1}{M}\int_{B}\tilde{\pi}(f)\dif\mu}{Z/M}=\frac{1}{Z}\int_{B}\tilde{\pi}(f)\dif\mu=\pi(B)
	\end{equation*}
	若$Q(B)=0$，则根据\cref{prop:ConditionalProbability}(1)可知$P(\{f\in B\}\mid C)=0$，并且由条件和\cref{prop:MeasurableIntegral}(7)(6)可得：
	\begin{equation*}
		0\leqslant\pi(B)=\frac{1}{Z}\int_{B}\tilde{\pi}(f)\dif\mu\leqslant\frac{M}{Z}\int_{B}q(f)\dif\mu=\frac{M}{Z}Q(B)=0
	\end{equation*}
	所以此时仍有$P(\{f\in B\}\mid C)=\pi(B)$。\par
	综上，单次试验中$f$在接受事件$C$发生条件下的条件分布为$\pi$。\par
	设$\{(f_n,U_n)\}$为各次相互独立且与$(f,U)$同分布的试验序列，记：
	\begin{equation*}
		C_n=\{U_n\leqslant\alpha(f_n)\},\quad T(x)=\inf\{n\in\mathbb{N}^+:x\in C_n\},\quad x\in X
	\end{equation*}
	由\cref{prop:Measure}(1)(3)（上连续性）和各次试验相互独立可得：
	\begin{equation*}
		P(T<+\infty)=1-P\left(\underset{n=1}{\overset{+\infty}{\cap}}\{T>n\}\right)=1-\lim_{n\to+\infty}\left(1-\frac{Z}{M}\right)^n=1
	\end{equation*}
	即算法以概率$1$在有限步内输出一个值$y$。
\end{proof}

%\subsection{多元分布}
%\subsubsection{Wishart分布}
%\begin{definition}
%	设$\mathbf{X_i}\text{i.i.d.}\sim\operatorname{N}_p(\mathbf{0},\Sigma),\;i=1,2,\dots,n,\;\Sigma>\mathbf{0},n\geqslant p$，记$\mathbf{X}=(\mathbf{X_1},\mathbf{X_2},\dots,\mathbf{X_n})^{\top}$，称随机矩阵：
%	\begin{equation*}
%		\mathbf{W}=\mathbf{X}^{\top}\mathbf{X}=\sum_{i=1}^{n}\mathbf{X_i}\mathbf{X_i}^{\top}
%	\end{equation*}
...
%		\begin{equation*}
%			C\mathbf{W}C^{\top}\sim\operatorname{W}_m(n, C\Sigma C^{\top})
%		\end{equation*}
%		\item 若$\mathbf{W}\sim\operatorname{W}_p(n,\Sigma)$，$\alpha\in\mathbb{R}^{p}$是任意常数向量，且$\alpha^{\top}\Sigma\alpha\ne0$，则：
%		\begin{equation*}
%			\frac{\alpha^{\top}\mathbf{W}\alpha}{\alpha^{\top}\Sigma\alpha}\sim\chi_n^2
%		\end{equation*}
%		\item 若$\mathbf{W}\sim\operatorname{W}_p(n,\Sigma)$，$\alpha\in\mathbb{R}^{p}$是任一非零常数向量，则：
%		\begin{equation*}
%			\frac{\alpha^{\top}\Sigma^{-1}\alpha}{\alpha^{\top}\mathbf{W}^{-1}\alpha}\sim\chi_{n-p+1}^2
%		\end{equation*}
%	\end{enumerate}
%\end{property}
%\begin{proof}
%	(1)由Wishart分布的定义和：
%	\begin{equation*}
%		\sum_{i=1}^{m}\mathbf{W}_i=\sum_{i=1}^{m}\sum_{j=1}^{n_i}\mathbf{X_{ij}}\mathbf{X_{ij}}^{\top}
%	\end{equation*}
%	立即可得。\par
%	(2)因为$C\mathbf{X}^{\top}=(C\mathbf{X_1},C\mathbf{X_2},\dots,C\mathbf{X_n})$，由\cref{prop:MultiNormal}(2)可得$C\mathbf{X_i}\sim\operatorname{N}_m(\mathbf{0},C\Sigma C^{\top})$且相互独立。由定义即可得到：
%	\begin{equation*}
%		C\mathbf{W}C^{\top}=C\mathbf{X}^{\top}\mathbf{X}C^{\top}\sim\operatorname{W}_m(n,C\Sigma C^{\top})
%	\end{equation*}\par
%	(3)注意到：
%	\begin{equation*}
%		\alpha^{\top}\mathbf{W}\alpha=\sum_{i=1}^{n}\alpha^{\top}\mathbf{X_i}\mathbf{X_i}^{\top}\alpha=\sum_{i=1}^{n}(\alpha^{\top}\mathbf{X_i})^2
%	\end{equation*}
%	由\cref{theo:MatNormalLinearTransform}可知$\alpha^{\top}\mathbf{X_i}\sim\operatorname{N}(0,\alpha^{\top}\Sigma\alpha)$且相互独立。因为$\Sigma>\mathbf{0},\;\alpha^{\top}\Sigma\alpha\ne0$，所以$\alpha^{\top}\Sigma\alpha>0$，即$(\alpha^{\top}\Sigma\alpha)^{-\frac{1}{2}}$存在。由\cref{theo:MatNormalLinearTransform}可知：
%	\begin{equation*}
%		\frac{\alpha^{\top}\mathbf{X_i}}{(\alpha^{\top}\Sigma\alpha)^{\frac{1}{2}}}\sim\operatorname{N}(0,1)
%	\end{equation*}
%	所以：
%	\begin{equation*}
%		\frac{\alpha^{\top}\mathbf{W}\alpha}{\alpha^{\top}\Sigma\alpha}=\frac{\sum\limits_{i=1}^{n}(\alpha^{\top}\mathbf{X_i})^2}{\alpha^{\top}\Sigma\alpha}=\sum_{i=1}^{n}\left(\frac{\alpha^{\top}\mathbf{X_i}}{(\alpha^{\top}\Sigma\alpha)^{\frac{1}{2}}}\right)^2\sim\chi_n^2
%	\end{equation*}\par
%	(4)不给予证明。
%\end{proof}
%\subsubsection{$T^2$分布}
%\begin{definition}
...
%	\begin{equation*}
%		T^2=\frac{n}{c}\mathbf{X}^{\top}\mathbf{W}^{-1}\mathbf{X}
%	\end{equation*}
...
%		\begin{equation*}
%			\frac{n}{c}(\mathbf{X}-\boldsymbol{\mu})^{\top}\mathbf{W}^{-1}(\mathbf{X}-\boldsymbol{\mu})\sim T^2(p,n)
%		\end{equation*}
