...
\end{definition}
\begin{method}
	由$\Sigma$可逆和\info{可逆半正定为正定}可知$\Sigma$为正定矩阵，根据\cref{theo:Cholesky}可知$\Sigma$有分解式$\Sigma=LL^T$且$L$可逆。由\cref{prop:InvertibleMatrix}(11)(12)和\cref{prop:Transpose}(4)可知：
	\begin{equation*}
		d^2_m(x,y)=(x-y)^T(LL^T)^{-1}(x-y)=(x-y)^T(L^T)^{-1}L^{-1}(x-y)=(x-y)^T(L^{-1})^TL^{-1}(x-y)=||L^{-1}(x-y)||_2^2
	\end{equation*}
