...
\begin{proof}
	...
	\begin{align*}
		&\int_{\mathbb{R}^{}}g(x)\dif Pf^{-1}=\int_{(\mathbb{R}^{}\setminus D)\cup\left(\underset{n=1}{\overset{+\infty}{\cup}}\{x_n\}\right)}g(x)\dif Pf^{-1} \\
		=&\int_{\mathbb{R}^{}\setminus D}g(x)\dif Pf^{-1}+\sum_{n=1}^{+\infty}\int_{\{x_n\}}g(x)\dif Pf^{-1}=\sum_{n=1}^{+\infty}g(x_n)P(f=x_n)\qedhere
	\end{align*}
\end{proof}
\begin{note}
	...
	\end{equation*}
	于是$\mathscr{A}$满足\cref{theo:SemiringMeasureExtension}中的条件，于是$\mu$在$\mathcal{B}=\sigma(\{(a,b]:a,b\in\mathbb{R}^{}\})$（\cref{prop:BorelSigmaField}(1.b)）上存在唯一的扩张$\nu$。在上述半环上，由$\mu$的定义可知：
	\begin{equation*}
		\mu\Bigl((a,b]\Bigr)=
		\begin{cases}
			F(b)-F(a),&a<b \\
	...
	\begin{equation*}
		...
		\begin{cases}
			P(\{f\leqslant b\}\setminus\{f\leqslant a\})=F(b)-F(a),&a<b \\
...
\begin{proof}
	...
	\end{align*}\par
	\textbf{(2)非负可测函数：}设$f$为非负可测函数，根据\cref{prop:MeasurableFunction}(8)，取非负简单函数列$\{f_n\}$满足$f_n\uparrow f$，由\cref{prop:Independent}(8)可知 $f_n$与$\mathscr{A}$独立。显然有$f_nI_A\uparrow fI_A$，根据\cref{prop:SimpleFunction}(3)(2)可知$f_nI_A$为非负简单函数，所以由\cref{prop:NonnegativeMeasurableIntegral}(4)、非负简单函数时的结论和\cref{prop:RSeq}(8.c)可得：
	\begin{align*}
