...
\begin{property}\label{prop:Expectation}
	...
	\begin{enumerate}
		...
		\end{equation*}
		\item 若$\seq{f}{n}$是$(X,\mathscr{F},P)$上相互独立的随机变量，$\seq{g}{n}$是$(\mathbb{R}^{},\mathcal{B}(\mathbb{R}^{}))$上的有限值可测函数，只要：
		\begin{equation*}
			\operatorname{E}\left[\prod_{i=1}^{n}g_i(f_i)\right]=\prod_{i=1}^{n}\operatorname{E}[g(f_i)]
		\end{equation*}
		之一有意义，二者一定相等。
	\end{enumerate}
\end{property}
...
\end{property}
\begin{proof}
	(1)必要性由条件期望的定义和\cref{prop:MeasurableIntegral}(11)即可得到，充分性由条件期望的定义和\cref{prop:MeasurableFunction}(9)即可得到。\par
	(2)因为$\sigma[\operatorname{E}(f)]=\{\varnothing,X\}\subseteq\mathscr{A}$，所以$\operatorname{E}(f)$作为$(X,\mathscr{A},P)$上的函数是可测的，由一般可测函数积分的定义可知$\operatorname{E}(f)$积分存在。分积分有限、积分为正无穷、积分为负无穷三种情况讨论，由积分的性质可得到$\int_{A}\operatorname{E}(f)\dif P=\operatorname{E}(f)P(A)$\info{回头再看看}。对于任意的$A\in\mathscr{A}$，因为$f$与$\mathscr{A}$独立，由\cref{lem:IndependentExpectation}、\cref{prop:MeasurableIntegral}(7)、\cref{prop:SigmaField}(4)和\cref{theo:MeasurableCountableIntegral}可得：
	\begin{equation*}
	...
	\end{equation*}
	由\cref{prop:MeasurableIntegral}(8)可知：
	\begin{align*}
	...
	\end{align*}\par
	(4)\textbf{$\;L_1$：}由\cref{prop:MeasurableIntegral}(8)可得：
	\begin{equation*}
	...
	\end{equation*}
	由\cref{prop:Measure}(4)（次有限可加性）、测度的非负性和\cref{prop:RSeq}(6)即可得出结论。\par
...
\begin{proof}
	...
	(2)由(1)和\cref{prop:MeasurableIntegral}(4)可知$f^i,\;i=0,1,\dots,n$在$X$上可积，即$\mu_i=\operatorname{E}(f)<+\infty$。由中心矩的定义和\cref{prop:MeasurableIntegral}(6)可得：
	\begin{equation*}
		\nu_n
