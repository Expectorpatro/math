...
随机数的生成通常以均匀分布为基础。数论中的同余等方法可以帮助我们在计算机上构造具有良好均匀性的伪随机数列，从而实现均匀分布的抽样。但对于概率密度较为复杂的分布，直接抽样往往并不容易。此时可以选取一个容易抽样的概率分布$Q$作为候选分布，先从$Q$中生成候选观测值，再按照适当的概率接受或拒绝该观测值。若接受概率选择得适当，则最终被接受的样本虽然最初来自$Q$，其条件分布却可以恰好等于目标分布$\pi$。这一思想对应的实现就是接受拒绝抽样法。
\begin{method}
	设$(Y,\mathscr{A},\mu)$是$\sigma$有限测度空间，$\tilde{\pi}:Y\to[0,+\infty)$是其上的可测函数，且：
	\begin{equation*}
		Z\coloneq\int_{Y}\tilde{\pi}(y)\dif\mu\in(0,+\infty)
	\end{equation*}
	在$(Y,\mathscr{A})$上定义目标概率测度$\pi$：
	\begin{equation*}
		\pi(B)\coloneq\frac{1}{Z}\int_{B}\tilde{\pi}(y)\dif\mu,\quad\forall\;B\in\mathscr{A}
	\end{equation*}
	设$Q$是$(Y,\mathscr{A})$上的概率测度，$Q\ll\mu$。根据\cref{theo:RadonNikodym}，取$q\coloneq\dfrac{\dif Q}{\dif\mu}$的一个非负版本（不讨论存在性）。若存在$M\in(0,+\infty)$使得：
	\begin{equation*}
	...
	\begin{equation*}
		...
		\end{cases}
	\end{equation*}
	在$\mu$零测集上适当修改$\alpha$，可使$0\leqslant\alpha(y)\leqslant1$对任意的$y\in Y$成立。设$f$为概率空间$(X,\mathscr{F},P)$到$(Y,\mathscr{A})$上的可测映射且$Pf^{-1}=Q$，$U\sim\operatorname{U}(0,1)$且$U$与$f$独立。接受--拒绝法按如下步骤生成一个观测值：
	\begin{enumerate}
...
\begin{proof}
	...
	\begin{equation*}
		C_n\coloneq\{U_n\leqslant\alpha(f_n)\},\quad T(x)\coloneq\inf\{n\in\mathbb{N}^+:x\in C_n\},\quad x\in X
	\end{equation*}
