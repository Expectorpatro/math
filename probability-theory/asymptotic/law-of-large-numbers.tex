...
\begin{note}
	...
	\end{enumerate}
	由函数项级数收敛的Cauchy式定义、\cref{prop:SigmaField}(2)和\cref{prop:MeasurableFunction}(5.b)(3)可以得到$\sum\limits_{n=1}^{+\infty}f_n$收敛是尾事件，根据\cref{prop:MeasurableFunction}(5.a)(6)和\cref{prop:RSeq}(8.b)可知如果$\lim\limits_{n\to+\infty}\dfrac{1}{n}\sum\limits_{i=1}^{n}f_i$存在，则$\lim\limits_{n\to+\infty}\dfrac{1}{n}\sum\limits_{i=1}^{n}f_i=\lim\limits_{n\to+\infty}\dfrac{1}{n}\sum\limits_{i=m}^{n}f_i$是尾随机变量。
\end{note}
...
\end{theorem}
\begin{proof}
	由\cref{cor:ChebyshevInequality}可知对任意的$\varepsilon>0$有：
	\begin{equation*}
