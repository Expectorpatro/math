...
\begin{proof}
	...
	\begin{align*}
		&\lim_{n\to+\infty}\left[\sum_{i=n}^{+\infty}\sum_{j=1}^{k_i}\mu(C_{ij})\right]=\lim_{n\to+\infty}\left[\sum_{i=1}^{+\infty}\sum_{j=1}^{k_i}\mu(C_{ij})-\sum_{i=1}^{n-1}\sum_{j=1}^{k_i}\mu(C_{ij})\right] \\
		=&\sum_{i=1}^{+\infty}\sum_{j=1}^{k_i}\mu(C_{ij})-\lim_{n\to+\infty}\sum_{i=1}^{n-1}\sum_{j=1}^{k_i}\mu(C_{ij})=0
	\end{align*}
	于是根据\cref{prop:RSeq}(8.b)可知：
	\begin{equation*}
...
\begin{property}\label{prop:Measure}
	...
	\begin{enumerate}
		...
		\item 若$\mathscr{B}$是$\mathscr{A}$的一个含有空集的子集，则$\mu$在$\mathscr{B}$上也成为一个测度。
	\end{enumerate}
\end{property}
\begin{proof}
...
\subsection{外测度}
\begin{definition}
	设$\tau$是$\mathscr{P}(X)$到$\overline{\mathbb{R}}$上的集函数，如果：
	\begin{enumerate}
...
\begin{property}\label{prop:ExteriorMeasure}
	$\mathscr{P}(X)$上的外测度$\tau$具有如下性质：
	\begin{enumerate}
		\item $\tau$是非负集函数；
		\item $\tau$具有次有限可加性；
		\item $\mathscr{P}(X)$上的测度$\tau$是$X$上的外测度。
	\end{enumerate}
\end{property}
\begin{proof}
	(1)由外测度的定义立即可得。\par
	...
	(3)显然$\mathscr{P}(X)$是一个$\sigma$域。由\cref{prop:SigmaField}(1)和\cref{theo:SetNecessarilySet1}可知$\mathscr{P}(X)$是一个半环，所以$\tau$是半环上的测度。由\cref{prop:Measure}(3)可知$\tau$具有单调性和次可列可加性。因为$\tau$是一个测度，所以$\tau(\varnothing)=0$。综上，$\tau$是$X$上的外测度。
\end{proof}
\begin{theorem}
	设$\mathscr{A}$是一个包含$\varnothing$的集族，$\mu$是$\mathscr{A}$上的一个非负集函数且满足$\mu(\varnothing)=0$，若对于任意的$A\in\mathscr{P}(X)$有：
	\begin{equation*}
...
\begin{proof}
	...
	(2)设$A\subseteq B$且$A,B\in\mathscr{P}(X)$，对于满足条件$B\subseteq\underset{n=1}{\overset{+\infty}{\cup}}B_n$的$\{B_n\}$，自然有$A\subseteq\underset{n=1}{\overset{+\infty}{\cup}}B_n$，所以：
	\begin{equation*}
		\tau(A)\leqslant\sum_{n=1}^{+\infty}\mu(B_n)
	\end{equation*}
	对右边取下确界即有$\tau(A)\leqslant\tau(B)$。\par
	(3)设$\{A_n\}$是$\mathscr{P}(X)$中的一个集合序列。\par
	若存在$n_0\in\mathbb{N}^+$使得$\tau(A_{n_0})=+\infty$，则$\sum\limits_{n=1}^{+\infty}\tau(A_n)=+\infty$。由(2)可得：
	\begin{equation*}
		\tau\left(\underset{n=1}{\overset{+\infty}{\cup}}A_n\right)=+\infty\leqslant\sum\limits_{n=1}^{+\infty}\tau(A_n)
	\end{equation*}
	即$\tau$具有次可列可加性。\par
	若$\tau(A_n)<+\infty$对$n\in\mathbb{N}^+$都成立，则：
	\begin{equation*}
		\left\{\sum_{i=1}^{+\infty}\mu(B_i):B_i\in\mathscr{A},\;A_n\subseteq\underset{i=1}{\overset{+\infty}{\cup}}B_i\right\}\ne\varnothing
	\end{equation*}
...
\begin{definition}[Caratheodory condition]
	设$X$是一个集合，$\tau$是$X$上的外测度。称满足条件：
	\begin{equation*}
		\tau(T)=\tau(T\cap A)+\tau(T\cap A^c),\;\forall\;T\in\mathscr{P}(X)
	\end{equation*}
	的集合$A\in\mathscr{P}(X)$为$\tau$可测集。将由所有$\tau$可测集构成的集族记作$\mathscr{A}_{\tau}$。
\end{definition}
\begin{lemma}\label{lem:EmeasureAB}
	设$X$是一个集合，$\tau$是$X$上的外测度。集合$E\in\mathscr{A}_{\tau}$的充要条件是对与$\forall\;A\subseteq E,\;\forall\;B\subseteq E^c$，总有：
	\begin{equation*}
...
\begin{property}\label{prop:tauMeasurableSetCollection}
	设$X$是一个集合，$\tau$是$X$上的外测度。$\mathscr{A}_{\tau}$具有如下性质：
	\begin{enumerate}
...
\begin{proof}
	...
	\end{equation}
	将\eqref{eq:TcapS_1measure}式代入\eqref{eq:S_1measure}式，再由\cref{prop:SetOperation}(7)可得：
	\begin{align*}
	...
	\end{align*}
	由于$T\cap S_1\subseteq S_1$，$(T\cap S_1^c)\cap S_2\subseteq S_1^c$，由\cref{lem:EmeasureAB}和\cref{prop:SetOperation}(4)可得：
	\begin{align*}
		&\tau(T\cap S_1)+\tau[(T\cap S_1^c)\cap S_2]=\tau[(T\cap S_1)\cup(T\cap S_1^c\cap S_2)] \\
		=&\tau\{T\cap[S_1\cup(S_1^c\cap S_2)]\}=\tau\{T\cap[(S_1\cup S_1^c)\cap(S_1\cup S_2)]\}=\tau[T\cap(S_1\cup S_2)]
	\end{align*}
	那么就有：
	\begin{equation*}
	...
	当$S_1\cap S_2=\varnothing$时有$S_2\subseteq S_1^c$，由\cref{lem:EmeasureAB}可得$\tau(S_1\cup S_2)=\tau(S_1)+\tau(S_2)$。\par
	...
	(6)设$\{S_n\}$互不相交。由(3)可得对任意的$n$，$\underset{i=1}{\overset{n}{\cup}}S_i\in\mathscr{A}_{\tau}$，那么对任意的$T\in\mathscr{P}(X)$，用\cref{prop:SetOperation}(4)和(3)可得：
	\begin{align*}
	...
	\end{align*}
	令$n\to+\infty$，根据\cref{prop:RSeq}(6)和\cref{prop:SetOperation}(4)可得：
	\begin{align}
	...
	\end{equation*}
	所以根据\cref{prop:ExteriorMeasure}(2)可得：
	\begin{equation*}
	...
	\begin{align*}
		\tau\left(\underset{n=1}{\overset{+\infty}{\cup}}S_n\right)\geqslant\sum_{i=1}^{+\infty}\tau\left[\left(\underset{n=1}{\overset{+\infty}{\cup}}S_n\right)\cap S_i\right]+\tau\left[\left(\underset{n=1}{\overset{+\infty}{\cup}}S_n\right)\cap\left(\underset{n=1}{\overset{+\infty}{\cup}}S_n\right)^c\right]=\sum_{i=1}^{+\infty} \tau(S_i)
	\end{align*}
	但是由外测度的次可列可加性有：
	\begin{equation*}
...
\begin{theorem}\label{theo:ExteriorMeasureCompleteSpace}
	设$X$是一个集合，$\tau$是$X$上的外测度。$(X,\mathscr{A}_{\tau},\tau)$是一个完全测度空间。
\end{theorem}
\begin{proof}
	...
	任取$A\in\mathscr{P}(X)$满足$\tau(A)=0$，对任何的$E\in\mathscr{P}(X)$，由\cref{prop:ExteriorMeasure}(1)可得$\tau(E\cap A)=0$，于是：
	\begin{equation*}
...
\begin{proof}
	...
	(3)任取$\mathscr{B}$中一个单调不减的集合序列$\{B_n\}$，则有$\mu(B_n)=\tau(B_n)$对任意的$n\in\mathbb{N}^+$成立，根据\cref{theo:SetNecessarilySet2}可知$\underset{n=1}{\overset{+\infty}{\cup}}B_n\in\sigma(\mathscr{A})$。令$B_0=\varnothing$，因为$\mu,\tau$是测度，所以$B_0\in\mathscr{B}$。由\cref{prop:SigmaField}(4)可知$B_n\setminus B_{n-1}\in\sigma(\mathscr{A})$，所以由\cref{prop:Measure}(2)可得：
	\begin{align*}
		...
		=&\sum_{n=1}^{+\infty}[\tau(B_n)-\tau(B_{n-1})]=\sum_{n=1}^{+\infty}\tau(B_n\setminus B_{n-1})=\tau\left[\underset{n=1}{\overset{+\infty}{\cup}}(B_n\setminus B_{n-1})\right]=\tau\left(\underset{n=1}{\overset{+\infty}{\cup}}B_n\right)
	\end{align*}
...
\begin{theorem}\label{theo:SemiringMeasureExtension}
	...
	\begin{equation*}
		\underset{n=1}{\overset{+\infty}{\cup}}A_n=X,\;\mu(A_n)<+\infty,\;\forall\;n\in\mathbb{N}^+
	\end{equation*}
	则$\tau$是唯一的且是由$\mu$生成的外测度，同时$\tau$是$\sigma$有限测度。
\end{theorem}
\begin{proof}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\end{equation*}
			\item $\mathscr{A}\subseteq\mathscr{A}_{\tau}$。
		\end{enumerate}
		\item 满足定理条件时$\tau$是唯一的。
	\end{enumerate}\par
	(1)取任意的$A\in\mathscr{A}$和任意满足$A\subseteq\underset{n=1}{\overset{+\infty}{\cup}}B_n$的$\mathscr{A}$中的集合序列$\{B_n\}$。根据半环的定义可得$A\cap B_n\in\mathscr{A}$，由\cref{prop:SetOperation}(4)和\cref{prop:Measure}(3)（次可列可加性和单调性）可得：
	\begin{equation*}
	...
	\begin{equation*}
		\sum_{n=1}^{+\infty}\mu(B_n)=\mu(A)\geqslant\tau(A)
	\end{equation*}
	于是有$\mu(A)=\tau(A)$。\par
	(2.a)因为$\mathscr{A}$是半环，由\cref{prop:SetOperation}(6)可知存在互不相交的$\seq{C}{n}\in\mathscr{A}$使得：
	\begin{equation*}
		B\cap A^c=B\setminus A=\underset{i=1}{\overset{n}{\cup}}C_i
	\end{equation*}
	由(1)可得$\mu(B)=\tau(B)$，根据\cref{prop:Measure}(1)可得：
	\begin{align*}
	...
	\end{equation*}
	根据(1)、(2.a)、\cref{prop:Series}(4)、外测度的定义和\cref{prop:SetOperation}(4)可得：
	\begin{align*}
	...
	\begin{equation*}
		\tau(C)=\tau(C\cap A)+\tau(C\cap A^c)
	\end{equation*}\par
	综上有$\mathscr{A}\subseteq\mathscr{A}_\tau$，由\cref{theo:ExteriorMeasureCompleteSpace}可得$\mathscr{A}_\tau$是$\sigma$域且$\tau$是$\mathscr{A}_\tau$上的测度，所以$\sigma(\mathscr{A})\subseteq\mathscr{A}_\tau$，$\tau$限制在$\sigma(\mathscr{A})$上时具有可列可加性，所以$\tau$是$\sigma(\mathscr{A})$上的测度。\par
	(3)因为半环是$\pi$系，由\cref{lem:PiExtensionMeasure}可知此时满足条件的$\tau$是唯一的。\par
	$\tau$的$\sigma$有限性由所给条件可直接得出。
\end{proof}
\begin{note}
	半环上测度的扩张结果实际为完全测度空间$(X,\mathscr{A}_\tau,\tau)$。
\end{note}
\begin{corollary}\label{cor:SemiringMeasureExtension}
	对于半代数$\mathscr{A}$上的$\sigma$有限测度$\mu$，存在$\sigma(\mathscr{A})$上$\mu$唯一的扩张$\tau$，且$\tau$是由$\mu$生成的外测度，同时$\tau$是$\sigma$有限测度。
\end{corollary}
\begin{proof}
	由\cref{theo:SemiringMeasureExtension}可知只需要证明存在互不相交的$\{A_n\}\subseteq\mathscr{A}$满足：
	\begin{equation*}
	...
	\end{equation*}
	其中$C_{ni}$互不相交。因为$C_{ni}\subseteq A_n$，由\cref{prop:Measure}(3)（单调性）可得$\mu(C_{ni})<+\infty$。$\{C_{ni}\}$即为满足条件的集合序列。
\end{proof}
\begin{theorem}\label{theo:DifOfAtauAndSigmaA}
	设$\tau$是半环$\mathscr{A}$上的测度$\mu$生成的外测度，则：
	\begin{enumerate}
		\item 对任意的$A\in\mathscr{A}_\tau$，存在$B\in\sigma(\mathscr{A})$使得$A\subseteq B$且$\tau(A)=\tau(B)$；
...
\begin{proof}
	...
	\end{equation*}
	令$n\to+\infty$，由\cref{prop:RSeq}(6)即可得到$\tau(B)\leqslant\tau(A)$，于是$\tau(A)=\tau(B)$。\par
	(2)由\cref{theo:SemiringMeasureExtension}的证明过程可知$\sigma(\mathscr{A})\subseteq\mathscr{A}_\tau$，所以$A_n\in\mathscr{A}_\tau$。对于任意的$A\in\mathscr{A}_\tau$，根据\cref{prop:tauMeasurableSetCollection}(4)可得$A\cap A_n\in\mathscr{A}_\tau$。由(1)可得存在$B_n\in\sigma(\mathscr{A})$满足$A\cap A_n\subseteq B_n$且$\tau(A\cap A_n)=\tau(B_n)$。由外测度的单调性和生成的外测度的定义可知$\tau(A\cap A_n)\leqslant\tau(A_n)\leqslant\mu(A_n)<+\infty$。令$B=\underset{n=1}{\overset{+\infty}{\cup}}B_n$，则根据\cref{prop:SetOperation}(4)可得：
	\begin{equation*}
	...
	\end{equation*}
	因为$\sigma(\mathscr{A})\subseteq\mathscr{A}_\tau$，所以$B_n\in\mathscr{A}_\tau$。根据\cref{theo:ExteriorMeasureCompleteSpace}可得$\mathscr{A}_\tau$是一个$\sigma$域且$\tau$是$\mathscr{A}_\tau$上的测度，由\cref{prop:Measure}(2)可得：
	\begin{equation*}
		\tau[B_n\setminus (A\cap A_n)]=\tau(B_n)-\tau(A\cap A_n)=0
	\end{equation*}
	由\cref{prop:SetOperation}(4)(5)、外测度的单调性和外测度的次可列可加性可得：
	\begin{align*}
		\tau(B\setminus A)&=\tau[B\setminus(A\cap X)]=\tau\left\{\left(\underset{n=1}{\overset{+\infty}{\cup}}B_n\right)\Big\backslash\left[A\cap\left(\underset{n=1}{\overset{+\infty}{\cup}}A_n\right)\right]\right\} \\
...
\begin{proof}
	...
	\end{equation*}
	由\cref{prop:SigmaField}(1)、\cref{theo:SetNecessarilySet1}和\cref{prop:Measure}(3)可得：
	\begin{equation*}
	...
	\textbf{(2)$\;\tilde{\mu}$是良定义的：}任取$A_1\cup N_1=A_2\cup N_2\in\tilde{\mathscr{A}}$。由$\tilde{\mu}$的定义有$\tilde{\mu}(A_1\cup N_1)=\mu(A_1)$，根据\cref{prop:Measure}(3)（次有限可加性）可得：
	\begin{equation*}
		\mu(A_1)\leqslant\mu(A_1\cup B_1\cup B_2)\leqslant\mu(A_1)+\mu(B_1)+\mu(B_2)=\mu(A_1)
	\end{equation*}
	所以$\mu(A_1\cup B_1\cup B_2)=\mu(A_1)$，于是根据\cref{prop:Measure}(3)（单调性）可得：
	\begin{equation*}
	...
	\textbf{(4)$\;(X,\tilde{\mathscr{A}},\tilde{\mu})$是完全测度空间：}若$C\subseteq(A\cup N)\in\tilde{\mathscr{A}},\;\tilde{\mu}(A\cup N)=0,\;A\cup N\in\tilde{\mathscr{A}}$，由\cref{prop:Measure}(3)（次有限可加性）可得：
	\begin{equation*}
...
\begin{theorem}\label{theo:CompletedExterior}
	...
	\begin{equation*}
		\underset{n=1}{\overset{+\infty}{\cup}}A_n=X,\;\mu(A_n)<+\infty,\;\forall\;n\in\mathbb{N}^+
	\end{equation*}
	则$(X,\mathscr{A}_\tau,\tau)$是$(X,\sigma(\mathscr{A}),\tau)$的完全化。
\end{theorem}
\begin{proof}
	记$\mathscr{C}=\sigma(\mathscr{A})$。\par
	\textbf{(1)$\;\tilde{\mathscr{C}}=\mathscr{A}_\tau$：}对任意的$A\cup N\in\tilde{\mathscr{C}},\;A\in\mathscr{C}$，由\cref{theo:ExteriorMeasureCompleteSpace}可知$(X,\mathscr{A}_\tau,\tau)$是一个完全测度空间，所以$N\in\mathscr{A}_\tau$。由\cref{theo:SemiringMeasureExtension}(2.b)和生成的定义可知$\mathscr{C}\subseteq\mathscr{A}_\tau$，所以$A\in\mathscr{A}_\tau$。根据\cref{prop:SigmaField}(3)可得$A\cup N\in\mathscr{A}_\tau$，所以$\tilde{\mathscr{C}}\subseteq\mathscr{A}_\tau$。\par
	对任意的$A\in\mathscr{A}_\tau$，由\cref{theo:DifOfAtauAndSigmaA}(2)可知存在$B\in\mathscr{C}$满足$A\subseteq B$且$\tau(B\setminus A)=0$。因为$\mathscr{C}\subseteq\mathscr{A}_\tau$，所以$B\in\mathscr{A}_\tau$，由\cref{prop:SigmaField}(4)可得$B\setminus A\in\mathscr{A}_\tau$，于是由\cref{theo:DifOfAtauAndSigmaA}(1)可知存在$C\in\mathscr{C}$满足$B\setminus A\subseteq C$且$\tau(C)=\tau(B\setminus A)=0$。因为$B\setminus A\subseteq C$，所以$(B\setminus A)\cap C^c=\varnothing$，于是由\cref{prop:SetOperation}(4)可得：
	\begin{align*}
		&A=(A\cap C)\cup(A\cap C^c)=(A\cap C)\cup[(A\cap C^c)\cup\varnothing] \\
		=&(A\cap C)\cup\{(A\cap C^c)\cup[(B\setminus A)\cap C^c]\}=(A\cap C)\cup\{[(B\setminus A)\cup A]\cap C^c\} \\
		=&(A\cap C)\cup(B\cap C^c)=(B\cap C^c)\cup(A\cap C)
	\end{align*}
	因为$\mathscr{C}\subseteq\mathscr{A}_\tau$，由\cref{prop:SigmaField}(2)可得$B\cap C^c\in\mathscr{C}$，而$A\cap C\subseteq C,\;\tau(C)=0$，所以$A\in\tilde{\mathscr{C}}$，于是有$\mathscr{A}_\tau\subseteq\tilde{\mathscr{C}}$。\par
	综上$\tilde{\mathscr{C}}=\mathscr{A}_\tau$。\par
	\textbf{(2)$\;\tau$：}对任意的$A\cup N\in\mathscr{A}_\tau,\;N\subseteq B\in\mathscr{C},\;\tau(B)=0$，由外测度的单调性和\cref{prop:ExteriorMeasure}(1)可得：
	\begin{equation*}
		\tau(A)\leqslant\tau(A\cup N)\leqslant\tau(A)+\tau(N)\leqslant\tau(A)+\tau(B)=\tau(A)
	\end{equation*}
	所以$\tau(A\cup N)=\tau(A)$。\par
	综上，由\cref{theo:CompleteMeasureSpace}可知$(X,\mathscr{A}_\tau,\tau)$是$(X,\sigma(\mathscr{A}),\tau)$的完全化。
\end{proof}

\subsection{乘积空间}
\begin{definition}
	设$\{(X_t,\mathscr{A}_t):t\in T\}$是一族可测空间，称：
	\begin{equation*}
	...
	\begin{equation*}
		\mathscr{D}=\underset{S\in\mathcal{D}}{\overset{}{\bigcup}}\left\{\pi_S^{-1}\left(\prod_{t\in S}^{}A_t\right)=\left(\prod_{t\in S}^{}A_t\right)\times\left(\prod_{t\notin S}^{}X_t\right):\forall\;t\in T,\;A_t\in\mathscr{A}_t\right\}
	\end{equation*}
	为\gls{FiniteDimensionalMeasurableCylinderSet}，记：
	\begin{equation*}
		\prod_{t\in T}^{}\mathscr{A}_t=\sigma(\mathscr{D})
	\end{equation*}
	并称$\sigma(\mathscr{D})$为$\{\mathscr{A}_t\}$的乘积，称$\left(\prod\limits_{t\in T}^{}X_t,\prod\limits_{t\in T}^{}\mathscr{A}_t\right)$为$\{(X_t,\mathscr{A}_t):t\in T\}$的乘积。
\end{definition}
\begin{property}
	有限维可测矩形柱集有如下性质：
	\begin{enumerate}
		\item 对任意的$n\in\mathbb{N}^+$和$S\in\mathcal{D}_n$，集族：
		\begin{equation*}
			\mathscr{D}_S\coloneq\left\{\prod_{t\in S}^{}A_t:\forall\;t\in S,\;A_t\in\mathscr{A}_t\right\}
		\end{equation*}
		是半环；
		\item $\mathscr{D}$是半代数；
	\end{enumerate}
\end{property}
\begin{proof}
	(1)对任意的$t\in S$取$A_t=\varnothing$，则：
	\begin{equation*}
		\prod_{t\in S}^{}A_t=\varnothing\in\mathscr{D}_S
	\end{equation*}
	任取$A,B\in\mathscr{D}_S$，有：
	\begin{equation*}
		A=\prod_{t\in S}^{}A_t,\;B=\prod_{t\in S}^{}B_t,\quad\forall\;t\in S,\;A_t,B_t\in\mathscr{A}_t
	\end{equation*}
	则由\cref{prop:SigmaField}(2)可得：
	\begin{equation*}
		A\cap B=\prod_{t\in S}^{}(A_t\cap B_t)\in\mathscr{D}_S
	\end{equation*}
	任取$A,B\in\mathscr{D}_S$，$B\subseteq S$，有：
	\begin{equation*}
		A=\prod_{t\in S}^{}A_t,\;B=\prod_{t\in S}^{}B_t,\quad\forall\;t\in S,\;A_t,B_t\in\mathscr{A}_t
	\end{equation*}
	则：
	\begin{equation*}
		A\setminus B=\left[\underset{C_t\in\{B_t,A_t\setminus B_t\}}{\overset{}{\bigcup}}\left(\prod_{t\in S}^{}C_t\right)\right]\Big\backslash\prod_{t\in S}^{}B_t
	\end{equation*}
	由\cref{prop:SigmaField}(4)可知$A_t\setminus B_t\in\mathscr{A}_t$，于是$A\setminus B$可以表示为$\mathscr{D}_S$中$2^n-1$个互不相交元素的并集。\par
	综上，由\cref{prop:Semiring}(2)可知$\mathscr{D}_S$是半环。\par
	(2)由定义即可得到$\prod\limits_{t\in T}^{}X_t\in\mathscr{D}$，下面只需证明$\mathscr{D}$是一个半环。\par
	对任意的$t\in S$取$A_t=\varnothing$，则：
	\begin{equation*}
	...
	综上，由\cref{prop:Semiring}(2)可知$\mathscr{D}$是半代数。
\end{proof}
\subsection{LS测度与L测度}
...
\begin{note}
	...
	\end{equation*}
	于是$\mathscr{A}$满足\cref{theo:SemiringMeasureExtension}中的条件，根据\cref{prop:BorelSigmaField}(1.b)可知$\mu$在$\mathcal{B}(\mathbb{R}^{})=\sigma(\{(a,b]:a,b\in\mathbb{R}^{}\})$上存在唯一的扩张$\lambda_F$，$\lambda_F$是$\mu$生成的外测度且是$\sigma$有限测度。
\end{note}
\begin{definition}
...
\begin{property}\label{prop:LSMeasure}
	LS测度$\lambda_F$有如下性质：
	\begin{enumerate}
		\item $\lambda_F$是由$F$引出的半环$\mathscr{A}=\{(a,b]:a,b\in\mathbb{R}^{}\}$上的测度$\mu$在$\mathcal{B}(\mathbb{R}^{})$上唯一的扩张，且是$\mu$生成的外测度，同时$\lambda_F$是$\sigma$有限测度；
		\item $(\mathbb{R}^{},\mathscr{A}_{\lambda_F},\lambda_F)$是一个完全测度空间，且是$(\mathbb{R}^{},\mathcal{B}(\mathbb{R}^{}),\lambda_F)$的完全化；
		\item $\mathscr{A}_{\lambda_F}$中包含所有$\lambda_F$零测集；
		\item 对任意的$A\in\mathscr{A}_{\lambda_F}$，存在$B\in\mathcal{B}(\mathbb{R}^{})$使得$A\subseteq B$且$\lambda_F(A)=\lambda_F(B)$；
		\item 对任意的$A\in\mathscr{A}_{\lambda_F}$，存在$B\in\mathcal{B}(\mathbb{R}^{})$使得$A\subseteq B$且$\lambda_F(B\setminus A)=0$。
	\end{enumerate}
\end{property}
\begin{proof}
	(1)由之前note中的讨论即可得到。\par
	(2)由(1)和\cref{theo:ExteriorMeasureCompleteSpace}可知$(\mathbb{R}^{},\mathscr{A}_{\lambda_F},\lambda_F)$构成一个完全测度空间，根据\cref{theo:CompletedExterior}即可得到$(\mathbb{R}^{},\mathscr{A}_{\lambda_F},\lambda_F)$是$(\mathbb{R}^{},\mathcal{B}(\mathbb{R}^{}),\lambda_F)$的完全化。\par
