...
\begin{note}
	...
	\end{equation*}
	于是$\mathscr{A}$满足\cref{theo:SemiringMeasureExtension}中的条件，于是$\mu$在$\mathcal{B}=\sigma(\{(a,b]:a,b\in\mathbb{R}^{}\})$（\cref{prop:BorelSigmaField}(1.b)）上存在唯一的扩张$\lambda_F$，$\lambda_F$是$\mu$生成的外测度。
\end{note}
\begin{definition}
...
\begin{proof}
	...
	(4)由\cref{theo:DifOfAtauAndSigmaA}(2)和(1)即可得到。\par
