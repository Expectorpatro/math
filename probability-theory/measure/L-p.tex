...
\subsection{$L_p$}
\begin{definition}
	设$(X,\mathscr{F},\mu)$是一个测度空间，$p\geqslant1$，$E\in\mathscr{F}$，$f,g$是$(E,E\cap\mathscr{F},\mu)$上的可测函数（\cref{prop:SigmaField}(6)）。定义等价关系$\sim$：$f\sim g\Leftrightarrow f=g\;$a.e.于$E$，将商空间：
	\begin{equation*}
...
\end{property}
\begin{proof}
	由模的三角不等式、\cref{ineq:else-1}和\cref{prop:MeasurableIntegral}(6)即可得出结论。
\end{proof}
