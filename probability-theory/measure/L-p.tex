...
\subsection{$L_p$}
\begin{definition}
	设$(X,\mathscr{F},\mu)$是一个测度空间，$1\leqslant p<+\infty$，$E\in\mathscr{F}$，$f,g$是$(E,E\cap\mathscr{F},\mu)$上的可测函数（\cref{prop:SigmaField}(6)(2)）。定义等价关系$\sim$：$f\sim g\Leftrightarrow f=g\;$a.e.于$E$，将商空间：
	\begin{equation*}
...
\begin{property}\label{prop:Lp}
	设$(X,\mathscr{F},\mu)$是一个测度空间，$1\leqslant p<+\infty$，$E\in\mathscr{F}$，则$L_p(E)$是$\mathbb{R}(\mathbb{C})$上的线性空间。
\end{property}
\begin{proof}
	由模的三角不等式、\cref{ineq:else1}和\cref{prop:MeasurableIntegral}(6)即可得出结论。
\end{proof}
...
下证明上式定义的距离满足距离公理：
\begin{proof}
	(1)$\;\rho\in R$：由\cref{ineq:else1}可得：
	\begin{equation*}
	...
	(4)三角不等式：设$x(t),y(t),z(t)\in L_p(E)$。$p=1$时可由绝对值的三角不等式立即得到，$1<p<+\infty$时，由\cref{ineq:Minkowski}可得：
	\begin{equation*}
...
\end{definition}
\begin{proof}
	(1)由$L_p(X,\mathscr{F},\mu)$的定义即可得到$||x||_p\in\mathbb{R}$。(2)非负性由\cref{prop:NonnegativeMeasurableIntegral}(2)(9)直接得到。(3)数乘由\cref{prop:NonnegativeMeasurableIntegral}(6)直接得到，(4)三角不等式的证明可由\cref{ineq:Minkowski}。再结合\cref{prop:Lp}即可得出结论。
\end{proof}
