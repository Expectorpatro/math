...
\begin{proof}
	...
	\begin{equation*}
		\frac{\psi(x)}{\varphi(x)}=\sum_{i=1}^{m}\sum_{j=1}^{n}\frac{d_j}{c_i}I_{E_i\cap F_j}(x)
	\end{equation*}\par
	\textbf{绝对值：}显然。\par
	\textbf{最大值最小值：}显然。\par
	(3)由$\sigma$域对补封闭即可得到：
...
\begin{property}\label{prop:MeasurableFunction}
	...
	\begin{enumerate}
		...
		\item $(\mathbb{R},\mathcal{B}(\mathbb{R}^{}))$上的实值单调函数是Borel函数；
		\item $(\mathbb{R}^n,\mathcal{B}(\mathbb{R}^{n}))$上的实值连续函数是Borel函数；
	\end{enumerate}
\end{property}
\begin{proof}
	...
	\begin{equation*}
		\forall\;n\in\mathbb{N}^+,\;\varphi_n(x)\leqslant\varphi_{n+1}(x)\leqslant f(x)
	\end{equation*}
...
\begin{definition}
	...
	\begin{equation*}
		F(x)=P(\{f\leqslant x\})
	\end{equation*}
	则称r.v.$\;f$的分布函数是$F$，也说成$f$服从$F$，记为$f\sim F$。
\end{definition}
\begin{definition}
