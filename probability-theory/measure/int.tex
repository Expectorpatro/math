...
\begin{proof}
	...
	\begin{align*}
		...
		&=\int_{A}|f(x)|\dif\mu
	\end{align*}\par
	(3)任取$B\subseteq A$且$B\in \mathscr{F}$。因为$f$在$A$上积分存在，所以$\int_{A}f^+(x)\dif\mu$和$\int_{A}f^-(x)\dif\mu$至少有一个有限。设$\int_{A}f^+(x)\dif x$有限，另一种情况可对称讨论。由\cref{prop:NonnegativeMeasurableIntegral}(5)、\cref{prop:SigmaField}(4)和\cref{prop:NonnegativeMeasurableIntegral}(2)可得：
	\begin{equation*}
	...
	只需证明$f\geqslant0\;$a.e.于$A$时的情况，对于$f\leqslant0\;$a.e.于$A$，只需取$-f$即可。\par
...
\end{proof}
\begin{theorem}
	设$f$是测度空间$(X,\mathscr{F},\mu)$上的可测函数。若$f$在$X$上积分存在，则对任一可列可测分割$\{A_n\}$有：
	\begin{equation*}
...
\begin{proof}
	...
	\begin{equation*}
		\int_{X}f_n^+(x)\dif\mu\Big\uparrow\int_{X}f^+(x)\dif\mu,\quad\int_{X}f_n^-(x)\dif\mu\Big\uparrow\int_{X}f^-(x)\dif\mu
	\end{equation*}
	...
	\begin{gather*}
		...
		\end{aligned} 
	\end{gather*}
	因为$f$在$X$上积分存在，根据\cref{prop:NonnegativeMeasurableIntegral}(5)(2)和上两式倒数第二个等号可知$f_n^+$和$f_n^-$在同一集合上的积分值至少有一个有限，于是由极限的线性性质可得：
	\begin{align*}
