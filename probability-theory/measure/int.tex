...
\begin{proof}
	...
	由\cref{prop:MeasurableFunction}(11)和(7)可知，可以改变$f$在零测集$\{f<0\}$上的值使得得到的函数$f'$为非负可测函数且满足$\int_{A}f'(x)\dif\mu=0$，由\cref{prop:NonnegativeMeasurableIntegral}(10)可知$f'=0\;$a.e.于$A$，于是根据\cref{prop:Measure}(1)可得：
	\begin{equation*}
		\mu(\{f\ne0\})=\mu(\{f'\ne0\}\cup\{f<0\})=\mu(\{f'\ne0\})+\mu(\{f<0\})=0
	\end{equation*}
	于是$f=0\;$a.e.于$A$。\par
	...
	(11)由(10)立即可得。\par
	(12)由(7)和\cref{prop:NonnegativeMeasurableIntegral}(4)证明\info{未完成}。
\end{proof}
