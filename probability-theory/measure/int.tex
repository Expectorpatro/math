...
\begin{property}\label{prop:NonnegativeMeasurableIntegral}
	...
	\begin{enumerate}
		...
		\end{equation*}
		\item 对任意的$A\in\mathscr{F},\;\alpha,\beta\in\mathbb{R}$，若$\alpha\int_{A}f(x)\dif\mu+\beta\int_{A}g(x)\dif\mu$有意义，则$\alpha f+\beta g$在$A$上积分存在，且：
		\begin{equation*}
...
\end{property}
\begin{proof}
	(1)任选$A$上的一个实值函数$f(x)$。因为$\mu(A)=0$，由\cref{prop:NonnegativeMeasurableIntegral}(3)可知：
	\begin{equation*}
	...
	\end{align*}\par
	(5)因为$f$是一个可测函数，由\cref{prop:MeasurableFunction}(7)可知$f^+,f^-$也是可测函数。由\cref{prop:MeasurableFunction}(2)可得$\{f^+=+\infty\},\{f^-=+\infty\}\in \mathscr{F}$。因为$f$可积，所以：
	\begin{equation*}
