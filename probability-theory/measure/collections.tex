\section{集合与集族}
\begin{definition}
	一个\textbf{集合}是指具有某种性质的事物的全体。组成集合的每个事物称为该集合的\textbf{元素}。若集合的元素只有有限多个，则称该集合为\textbf{有限集}，否则称为\textbf{无限集}；不含任何元素的集合称为\textbf{空集}，用符号$\varnothing$表示。语境中所有相关元素构成的集合称之为\textbf{全集}，一般用$X$表示，也称为空间或全空间。
\end{definition}
\begin{definition}
