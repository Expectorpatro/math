...
\end{equation*}
\begin{lemma}\label{lem:RadonNikodym1}
...
\begin{proof}
	...
	\end{equation*}
\end{proof}
\begin{lemma}\label{lem:RadonNikodym2}
...
\end{lemma}
\begin{proof}
	取满足\cref{lem:RadonNikodym1}中条件的$f$，对任意的$A\in\mathscr{F}$，令：
	\begin{equation*}
		\nu(A)=\varphi(A)-\int_{A}f(x)\dif\mu
	\end{equation*}
	由\cref{lem:RadonNikodym1}、\cref{prop:MeasurableIntegral}(1)和\cref{theo:MeasurableCountableIntegral}可知$\nu$是$(X,\mathscr{F})$上的测度。对任意的$n\in\mathbb{N}^+$和任意的$A\in\mathscr{F}$，定义：
	\begin{equation*}
		\nu_n(A)=\nu(A)-\frac{1}{n}\mu(A)
	\end{equation*}
	...
	\end{align*}
	所以$f(x)+\dfrac{1}{n}I(x\in X_n^+)\in\mathscr{F}$。取$A=X$，根据\cref{lem:RadonNikodym1}可得：
	\begin{align*}
...
\end{proof}
\begin{lemma}\label{lem:RadonNikodym3}
...
\end{lemma}
\begin{proof}
	先讨论$\varphi$是有限符号测度的情况，此时$\varphi$的Jordan分解$\varphi^+$和$\varphi^-$都是$(X,\mathscr{F})$上的有限测度。对任意满足$\varphi(A)=0$的$A\in\mathscr{F}$，由\cref{prop:Measure}(3)可知$\varphi^+(A)=\varphi^-(A)=0$，所以$\varphi^+,\varphi^-\ll\mu$。根据\cref{lem:RadonNikodym2}可知存在非负的$f^+,f^-\in L_1(X,\mathscr{F},\mu)$使得：
	\begin{equation*}
	...
	\end{align*}
\end{proof}
\begin{lemma}\label{lem:RadonNikodym4}
...
\begin{proof}
	...
	\end{equation*}
	由\cref{theo:CountableUnionCountable}可知$\mathscr{D}$对可列不交并封闭。任取$A,B\in\mathscr{D}$，则存在互不相交的$\{A_n\}\subseteq\mathscr{F}$和互不相交的$\{B_n\}\subseteq\mathscr{F}$满足：
	\begin{equation*}
	...
	因为$\varphi$和$\mu$分别是$(B,B\cap\mathscr{F})$上的$\sigma$有限符号测度和有限测度，$\varphi\ll\mu$，由\cref{lem:RadonNikodym3}可知存在有限的可测函数$g$使得对任意的$A\in B\cap\mathscr{F}$有：
	\begin{equation*}
...
\end{proof}
\begin{theorem}[Radon-Nikodym Theorem]\label{theo:RadonNikodym}
	...
	\end{equation*}
	且$f$在a.e.于$(X,\mathscr{F},\mu)$的意义下唯一。若$\varphi$是$\sigma$有限的，则$f$也是有限的。若$\varphi$是测度，则$f\geqslant0\;$a.e.于$(X,\mathscr{F},\mu)$。称满足上式条件的$f$为$\varphi$关于$\mu$的Radon-Nikodym导数，记作$\dfrac{\dif\varphi}{\dif\mu}$。
\end{theorem}
\begin{proof}
	因为$\mu$是$\sigma$有限测度，所以可取$(X,\mathscr{F})$的一个可列可测分割$\{A_n\}$，满足对任意的$n\in\mathbb{N}^+$有$\mu(A_n)<+\infty$。由\cref{prop:Measure}(3)（单调性）和\cref{lem:RadonNikodym4}可知存在$(A_n,A_n\cap\mathscr{F})$上的可测函数$f_n$使得对任意的$A\in A_n\cap\mathscr{F}$有：
	\begin{equation*}
		\int_{A}f_n^-(x)\dif\mu<+\infty,\quad\varphi(A)=\int_{A}f_n(x)\dif\mu
	\end{equation*}
	同时$f_n$在a.e.于$(A_n,A_n\cap\mathscr{F},\mu)$的意义下唯一，若$\varphi$在$(A_n,A_n\cap\mathscr{F})$上是$\sigma$有限的，则$f_n$也是有限的。取$f=\sum\limits_{n=1}^{+\infty}f_n$，与\cref{lem:RadonNikodym3}完全一样的过程可以证明得到$f$是一个可测函数，在a.e.于$(X,\mathscr{F},\mu)$的意义下唯一且满足：
	\begin{equation*}
		\forall\;A\in\mathscr{F},\;\varphi(A)=\int_{A}f(x)\dif\mu,\;\int_{A}f^-(x)\dif\mu<+\infty
	\end{equation*}
	若$\varphi$是$\sigma$有限的，则由\cref{prop:SignedMeasure}(1)(4)可得$\varphi$在$(A_n,A_n\cap\mathscr{F})$上是$\sigma$有限的，所以$f_n$在$A_n$上是有限的，于是$f$在$X$上是有限的。若$\varphi$是测度，取$N=\{f<0\}$，则：
	\begin{equation*}
		N=\underset{n=1}{\overset{+\infty}{\cup}}\left\{f<-\frac{1}{n}\right\}
	\end{equation*}
	令$N_n=\left\{f<-\frac{1}{n}\right\}$，由\cref{prop:MeasurableFunction}(1.b)和\cref{prop:MeasurableIntegral}(7)可知：
	\begin{equation*}
		\varphi(N_n)=\int_{N_n}f(x)\dif\mu\leqslant\int_{N_n}-\frac{1}{n}\dif\mu=-\frac{\mu(N_n)}{n}\leqslant0
	\end{equation*}
	因为$\varphi$是测度，所以$\varphi(N_n)=0$，即$\mu(N_n)=0$。根据\cref{prop:Measure}(3)次有限可加性可知$\mu(N)=0$，即$f\geqslant0\;$a.e.于$(X,\mathscr{F},\mu)$。
\end{proof}
\begin{lemma}\label{lem:IntChangeOfMeasure}
	...
	\begin{equation*}
		\int_{A}f(x)\dif\varphi,\quad\int_{A}f(x)\frac{\dif\varphi}{\dif\mu}(x)\dif\mu
	\end{equation*}
	之一有意义，二者一定相等。
\end{lemma}
\begin{proof}
	...
	\end{align*}
\end{proof}
\begin{property}\label{prop:RadonNikodym}
	设$(X,\mathscr{F})$是一个可测空间。Radon-Nikodym导数的计算具有如下性质：
	\begin{enumerate}
		...
		\end{equation*}
		\item 若$\varphi$是$(X,\mathscr{F})$上的符号测度，$\psi,\mu$是$(X,\mathscr{F})$上的$\sigma$有限测度，$\varphi\ll\psi\ll\mu$，则：
		\begin{equation*}
			\frac{\dif\varphi}{\dif\psi}=\frac{\dif\varphi}{\dif\mu}\Big/\frac{\dif\psi}{\dif\mu}
		\end{equation*}
	\end{enumerate}
\end{property}
\begin{proof}
	(1)因为$\varphi$是符号测度，$\nu$是$\sigma$有限测度且$\varphi\ll\nu$，根据\cref{theo:RadonNikodym}可知存在$(X,\mathscr{F},\nu)$上在a.e.意义下唯一的可测函数$\dfrac{\dif\varphi}{\dif\nu}$使得：
	\begin{equation*}
	...
	\end{equation*}
	因为$\varphi$是符号测度，$\mu$是$\sigma$有限测度且$\varphi\ll\mu$，根据\cref{theo:RadonNikodym}可知存在$(X,\mathscr{F},\mu)$上在a.e.意义下唯一的可测函数$\dfrac{\dif\varphi}{\dif\mu}$使得：
	\begin{equation*}
	...
	\end{equation*}\par
	(2)因为$\nu\ll\mu$且$\nu$和$\mu$是$\sigma$有限测度，根据\cref{theo:RadonNikodym}可知存在$(X,\mathscr{F},\mu)$上在a.e.意义下唯一的可测函数$\dfrac{\dif\nu}{\dif\mu}$使得：
	\begin{equation*}
	...
	\begin{equation*}
		\forall\;A\in\mathscr{F},\;\nu(A)=\int_{A}\frac{\dif\nu}{\dif\mu}(x)\dif\mu=\int_{A}\frac{\dif\nu}{\dif\mu}(x)\cdot\frac{\dif\mu}{\dif\nu}(x)\dif\nu
	\end{equation*}
	而：
	\begin{equation*}
	...
	(3)根据\cref{prop:SignedMeasure}(5)可知$a\varphi$也是一个符号测度。因为$\varphi\ll\mu$，由$a\varphi$的定义可得$a\varphi\ll\mu$，根据\cref{theo:RadonNikodym}可知存在$(X,\mathscr{F},\mu)$上在a.e.意义下唯一的可测函数$\dfrac{\dif(a\varphi)}{\dif\mu}$使得：
	\begin{equation*}
	...
	\end{equation*}\par
	(4)因为$\varphi,\psi\ll\mu$，根据$\varphi+\psi$的定义可得$\varphi+\psi\ll\mu$。由\cref{theo:RadonNikodym}可知存在$(X,\mathscr{F},\mu)$上在a.e.意义下唯一的可测函数$\dfrac{\dif(\varphi+\psi)}{\dif\mu}$使得：
	\begin{equation*}
	...
	\end{equation*}
	由\cref{theo:RadonNikodym}中Radon-Nikodym导数负部的可积性和\cref{prop:MeasurableIntegral}(6)可得：
	\begin{equation*}
	...
	\begin{equation*}
		\frac{\dif(\varphi+\psi)}{\dif\mu}=\frac{\dif\varphi}{\dif\mu}+\frac{\dif\psi}{\dif\mu}\;\text{a.e.于}(X,\mathscr{F},\mu)
	\end{equation*}\par
	(5)对任意的$A\in\mathscr{F}$，由\cref{theo:RadonNikodym}、\cref{prop:MeasurableFunction}(5.c)和\cref{lem:IntChangeOfMeasure}可得：
	\begin{equation*}
		\int_{A}\frac{\dif\varphi}{\dif\mu}(x)\Big/\frac{\dif\psi}{\dif\mu}(x)\dif\psi=\int_{A}\frac{\dif\varphi}{\dif\mu}(x)\Big/\left[\frac{\dif\psi}{\dif\mu}(x)\frac{\dif\psi}{\dif\mu}(x)\right]\dif\mu=\int_{A}\frac{\dif\varphi}{\dif\mu}(x)\dif\mu=\varphi(A)
	\end{equation*}
	根据\cref{theo:RadonNikodym}可得：
	\begin{equation*}
		\frac{\dif\varphi}{\dif\mu}\Big/\frac{\dif\psi}{\dif\mu}=\frac{\dif\varphi}{\dif\psi}\qedhere
	\end{equation*}
\end{proof}
...
\end{lemma}
\begin{proof}
	由定义可验证得到$\varphi+\mu$是一个测度，根据测度的非负性可知$\varphi\ll\varphi+\mu$，由\cref{theo:RadonNikodym}可知对任意的$A\in\mathscr{F}$有：
	\begin{equation*}
