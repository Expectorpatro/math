...
\begin{theorem}[Jordan Decomposition]
	...
	设$\varphi$是可测空间$(X,\mathscr{F})$上的符号测度，$\{X^+,X^-\}$为$\varphi$在$X$上的Hahn分解，则存在$(X,\mathscr{F})$上的测度$\varphi^+=\varphi(A\cap X^+)$和有限测度$\varphi^-=-\varphi(A\cap X^-)$使得$\varphi=\varphi^+-\varphi^-$并且有：
	\begin{equation*}
