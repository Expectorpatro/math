...
\begin{lemma}\label{lem:RandonNikodym2}
	设$\varphi$和$\mu$都是可测空间$(X,\mathscr{F})$上的有限测度，$\varphi\ll\mu$，则存在非负的$f\in L_1(X,\mathscr{F},\mu)$使得：
	\begin{equation*}
		\varphi(A)=\int_{A}f(x)\dif\mu,\quad\forall\;A\in\mathscr{F}
	\end{equation*}
...
\begin{lemma}\label{lem:RandonNikodym3}
	设$\varphi$和$\mu$分别是可测空间$(X,\mathscr{F})$上的$\sigma$有限符号测度和有限测度，$\varphi\ll\mu$，则存在有限的可测函数$f$使得$\int_{A}f^-(x)\dif\mu<+\infty$且：
	\begin{equation*}
		\varphi(A)=\int_{A}f(x)\dif\mu,\quad\forall\;A\in\mathscr{F}
	\end{equation*}
