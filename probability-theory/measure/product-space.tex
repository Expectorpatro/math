...
\begin{definition}
	...
	\begin{enumerate}
		...
		\end{equation*}
	\end{enumerate}
	则称$F$为$\mathbb{R}^{n}$上的\textbf{准分布函数}。
\end{definition}
\begin{theorem}\label{theo:MultiDimensionalQuasi-distributionMeasure}
...
\subsection{多维分布函数}
\begin{definition}
	若$\mathbb{R}^{n}$上的准分布函数$F$满足：
	\begin{equation*}
		\lim_{\seq{x}{n}\to-\infty}F(\seq{x}{n})=0,\quad\lim_{\seq{x}{n}\to+\infty}F(\seq{x}{n})=1
	\end{equation*}
	则称$F$是$\mathbb{R}^{n}$上的\textbf{n元分布函数}。
\end{definition}
\begin{theorem}\label{theo:ProductBorelSet}
	$\mathcal{B}(\mathbb{R}^{n})=\prod\limits_{i=1}^{n}\mathcal{B}(\mathbb{R}^{})$。
\end{theorem}
\begin{definition}
	若$f$是可测空间$(X,\mathscr{F})$到$(\mathbb{R}^{n},\mathcal{B}(\mathbb{R}^{n}))$上的可测映射，则称$f$是\gls{RandomVector}。
\end{definition}
\begin{theorem}
	设$f=(\seq{f}{n})$是概率空间$(X,\mathscr{F},P)$上的随机向量。对任意的$x=(\seq{x}{n})\in\mathbb{R}^{n}$，令：
	\begin{equation*}
		F(x)=P\left(\underset{i=1}{\overset{n}{\cap}}\{f_i\leqslant x_i\}\right)
	\end{equation*}
