...
%\begin{definition}
%	样本集$E$由$n$类不同的样本构成，第$i$类样本在$E$中所占的比例为$p_i,\;i=1,2,\dots,n$，定义$-\sum\limits_{i=1}^{n}p_i\log(p_i)$为该样本集的信息熵，记作$\operatorname{Ent}(E)$。
%\end{definition}
%由样本集信息熵的定义可以看出
%\begin{definition}
%	假设离散特征$a$有$n$个取值，分别为$\seq{a}{n}$。使用$a$来对样本集$E$进行划分，即将$E$分成$n$份，其中第$i$份包含了$E$中所有在特征$a$上取值为$a_i$的样本，记为$E^i$，$i=1,2,\dots,n$。定义：
%	\begin{equation*}
%		\operatorname{Gain}(E,a)=\operatorname{Ent}(E)-\sum_{i=1}^{n}\frac{|E_i|}{|E|}\operatorname{Ent}(E_i)
%	\end{equation*}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State \Return \(F^{\star}, \text{BestScore}\)
		\EndFunction
	\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State \textbf{Output:} 剪枝后的决策树 \( T^{\star} \)
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State \textbf{Output:} 最优子树 $T^{\star}$
		...
		\]
		\EndFor
		\State 令 $\alpha_{\min} = \min \alpha_t$，将 $\alpha_{\min}$ 加入序列 $\alpha$，找到其对应节点 $t^{\star}$，将 $T_{t^{\star}}$ 剪去，令其变为叶节点，得到新树 $T'$
		\State 更新 $T \gets T'$ 并加入候选树集：$\mathcal{T} \gets \mathcal{T} \cup \{T\}$
		\EndWhile

		\State 令 $T^{\star} = \arg\min_{T_i \in \mathcal{T}}\text{Error}(T_i, \mathcal{D}_{val})$，其中$\text{Error}(T_i, \mathcal{D}_{val})$是树$T_i$在验证集$\mathcal{D}_{val}$上的误差
		\State \Return $T^{\star}$
		\EndFunction
	\end{algorithmic}
\end{algorithm}
