...
\begin{definition}
	...
	\end{equation*}
	则称映射$T$在点$x_0$处Fr\'{e}chet \gls{Differentiable}，称$A$是$T$在点$x_0$处的Fr\'{e}chet \gls{Differential}，记作$\operatorname{D}Tx_0=A$。若$T$在$E$中的每一个点处都可微，则称$T$在$E$上可微。若$\operatorname{D}T$在$x_0$处Fr\'echet可微，将其Fr\'echet微分记作$\operatorname{D}^2T$，更高阶的情况可依此类推。
\end{definition}
\begin{property}\label{prop:FrechetDifferential}
	...
	\begin{enumerate}
		...
		\item 若$T,S$在$x_0$处Fr\'echet可微，则对于任意的$\alpha,\beta\in K$，有：
		\begin{equation*}
			\operatorname{D}(\alpha Tx_0+\beta Sx_0)=\alpha \operatorname{D}Tx_0+\beta\operatorname{D}Sx_0
		\end{equation*}
...
\begin{proof}
	...
	\end{equation*}
	则对于任意的$h\in X\backslash\{\mathbf{0}\}$有：
	\begin{align*}
	...
	\end{align*}
	所以$||(B-A)h||=0$对任意的$h\in X\backslash\{\mathbf{0}\}$都成立，即$A=B$。\par
	...
	(3)由(2)立即可得。\par
	(4)令$A=\operatorname{D}Tx_0,\;B=\operatorname{D}Sx_0$。对于任意的$h\in X\backslash\{\mathbf{0}\}$有：
	\begin{align*}
	...
	\end{equation*}
	结论成立。\par
	(5)对于任意的$h\in X\backslash\{\mathbf{0}\}$有：
	\begin{align*}
...
\begin{property}\label{prop:FrechetGateaux}
	设$X,Y$为数域$K$上的赋范线性空间，$E\subseteq X$是一个开集，$x_0\in E$，$T:E\to Y$是一个映射。若$f$在$x_0$处Fr\'echet可微，则它在$x_0$处沿任何方向$h$的G\^ateaux方向导数都存在，且$\dif T(x_0;h)=(\operatorname{D}Tx_0)h$。
\end{property}
\begin{proof}
	由Fr\'echet可微和G\^teaux方向导数的定义立即可得。
\end{proof}
