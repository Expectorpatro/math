...
\begin{proof}
	...
	\end{equation*}
	上升最快即$(\operatorname{D}fx_0)h$最大，下降最快即$(\operatorname{D}fx_0)h$最小。根据\cref{ineq:cauchy-schiwarz-inner-product}可得：
	\begin{equation*}
		|(\operatorname{D}fx_0)h|^2=|(\nabla fx_0,h)|^2\leqslant(\nabla fx_0,\nabla fx_0)(h,h)=||\nabla fx_0||^2
	\end{equation*}
	等号成立当且仅当$\nabla fx_0$与$h$共线，同向时$(\operatorname{D}fx_0)h>0$且取到最大值，反向时$(\operatorname{D}fx_0)h<0$且取到最小值。
\end{proof}

\section{定积分}
\begin{definition}
	设$Q_i=[a_i,b_i]\subseteq\mathbb{R}^{},\;i=1,2,\dots,n$，$Q=Q_1\times Q_2\times\cdots\times Q_n\subseteq\mathbb{R}^{n}$是闭方块。称$Q_i$为$Q$的\gls{Edge}，定义$Q$的\gls{Volume}为：
	\begin{equation*}
...
\end{definition}
\begin{definition}
	设$Q^i=[a^i,b^i]\subseteq\mathbb{R}^{},\;i=1,2,\dots,n$，$Q=Q^1\times Q^2\times\cdots\times Q^n\subseteq\mathbb{R}^{n}$是闭方块，$Q^i$的分割为：
	\begin{equation*}
...
\begin{definition}
	...
	\end{equation*}
	则称$\{P_n\}$是\gls{SequenceOfPartitionsWithMeshTendingToZero}。
\end{definition}
\begin{definition}
	设$Q\subseteq\mathbb{R}^{n}$是闭方块，$f:Q\to\mathbb{R}^{}$是函数，对于$Q$的任意一个分割：
	\begin{equation*}
		P=\{P^1,P^2,\dots,P^n\}
	\end{equation*}
...
\end{definition}
\begin{definition}
	设$Q\subseteq\mathbb{R}^{n}$是闭方块，$f:Q\to\mathbb{R}^{}$是函数，$I\in\mathbb{R}^{}$。若对任意的$\varepsilon>0$，存在$\delta>0$使得只要$|P|<\delta$，不论分割$P$对应的标志点组$\xi$如何选择，都有：
	\begin{equation*}
