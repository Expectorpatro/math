...
\begin{definition}
	...
	\end{equation*}
	称$P=\{P^1,P^2,\dots,P^n\}$为$Q$的一个分割，称$|P|=\max\{|P^i|:i=1,2,\dots,n\}$为分割$P$的模。设$x_{i_j}^i$为$Q^i$的一个分点，记：
	\begin{equation*}
...
\end{definition}
\begin{definition}
	如果闭方块$Q\subseteq\mathbb{R}^n$上的分割序列$\{P_n\}$满足：
	\begin{equation*}
...
\begin{theorem}\label{theo:RiemannIntegrableBounded}
	设函数$f$在闭方块$Q\subseteq\mathbb{R}^n$上可积，则$f$在$Q$上有界。
\end{theorem}
\begin{proof}
...
\end{proof}
\begin{definition}
	设函数$f$在闭方块$Q\subseteq\mathbb{R}^n$上有定义且有界。由\info{确界原理}可知，对于$Q$的一个分割$P$，$f$在$Q$的任一闭子方块$Q_J$上都有有限的下确界和上确界，记为：
	\begin{equation*}
...
\end{definition}
\begin{definition}
	设函数$f$在闭方块$Q\subseteq\mathbb{R}^n$上有定义且有界，$P$是$Q$上的一个分割，分别称：
	\begin{equation*}
...
\begin{property}\label{prop:DarbouxSum}
	设函数$f$在闭方块$Q\subseteq\mathbb{R}^n$上有定义且有界，$P$是$Q$上的一个分割，$f$关于$P$的Darboux下和$L(f,P)$和Darboux上和$U(f,P)$具有如下性质：
	\begin{enumerate}
		\item $-L(-f,P)=U(f,P)$；
...
\begin{proof}
	...
	\begin{equation*}
		\{x=(x^1,x^2,\dots,x^n)\in Q:x^h=\alpha,\;\alpha\in(x^h_{k-1},x^h_k)\}
	\end{equation*}
	注意到：
	\begin{align*}
		L(f,P)=\sum_{J:h_j\ne k}m_J\Delta x_J+\sum_{J:h_j=k}m_J\Delta x_J
	\end{align*}
	添加分界后改变的部分仅为后一项，且添加分界后，后一项中的每一个部分为：
	\begin{equation*}
		m_{J'}\left(\frac{\alpha-x^h_{k-1}}{\Delta x^h_k}\right)\Delta x_J+m_{J''}\left(\frac{x^h_k-\alpha}{\Delta x^h_k}\right)\Delta x_J
	\end{equation*}
	于是：
	\begin{align*}
		&L(f,P')-L(f,P)=\sum_{J:h_j=k}\left[m_{J'}\left(\frac{\alpha-x^h_{k-1}}{\Delta x^h_k}\right)\Delta x_J+m_{J''}\left(\frac{x^h_k-\alpha}{\Delta x^h_k}\right)\Delta x_J-m_J\Delta x_J\right] \\
		=&\sum_{J:h_j=k}\left[m_{J'}\left(\frac{\alpha-x^h_{k-1}}{\Delta x^h_k}\right)\Delta x_J+m_{J''}\left(\frac{x^h_k-\alpha}{\Delta x^h_k}\right)\Delta x_J-m_J\left(\frac{\alpha-x^h_{k-1}+x^h_k-\alpha}{\Delta x^h_k}\right)\Delta x_J\right] \\
		=&\sum_{J:h_j=k}\left[(m_{J'}-m_J)\left(\frac{\alpha-x^h_{k-1}}{\Delta x^h_k}\right)\Delta x_J+(m_{J''}-m_J)\left(\frac{x^h_k-\alpha}{\Delta x^h_k}\right)\Delta x_J\right]
	\end{align*}
	注意到$m_J\leqslant m_{J'},m_{J''}\leqslant M_J$，所以有：
	\begin{equation*}
		0\leqslant L(f,P')-L(f,P)\leqslant\sum_{J:h_j=k}\omega_J\Delta x_J\leqslant\omega\sum_{J:h_j=k}\Delta x_J=\omega\frac{\Delta x^h_k}{b^h-a^h}\operatorname{Vol}(Q)\leqslant\omega C|P|
	\end{equation*}
	即：
	\begin{equation*}
...
\begin{theorem}\label{theo:RiemannIntegrableIff}
	设函数$f$在方块$Q\subseteq\mathbb{R}^n$上有定义并且有界，则下述三条结论等价：
	\begin{enumerate}
...
\begin{theorem}\label{theo:RiemannIntegrable4ArithmeticOperation}
	设函数$f,g$在闭方块$Q\subseteq\mathbb{R}^n$上可积，$\alpha,\beta\in\mathbb{R}$，则：
	\begin{enumerate}
...
\begin{theorem}\label{theo:RiemannIntegrable}
	...
	\begin{enumerate}
		...
		\item 若函数$f$在闭方块$Q\subseteq\mathbb{R}^n$上连续，则$f$在$Q$上可积；
...
\begin{proof}
	...
	\end{equation*}\par
	(3)因为$f$在$Q$上连续，由\cref{theo:CompactRn}和\cref{prop:CompactMap}(4)可得$f$在$Q$上一致连续，所以对任意的$\varepsilon>0$，存在$\delta>0$，当$|x-y|<\delta$时就有$|f(x)-f(y)|<\varepsilon$。取$|P|<\delta$，于是：
	\begin{equation*}
	...
	\begin{equation*}
		x,y\in[a,b]\Big\backslash\left(\underset{j=1}{\overset{l}{\cup}}J_j\right),\;|x-y|<\delta
	\end{equation*}
	...
	\begin{align*}
		\Omega(f,P)&=\sum_s'\omega_s\Delta x_s+\sum_t''\omega_t\Delta x_t<\eta\sum_{s}'\Delta x_s+\omega\sum_{t}''\Delta x_t \\
		&\leqslant\eta(b-a)+4l\omega\eta=(b-a+4l\omega)\eta
	\end{align*}
...
\begin{property}
	...
	\begin{enumerate}
		...
		\end{equation*}
		\item 若函数$f,g$在闭方块$Q\subseteq\mathbb{R}^n$上可积且$f\leqslant g$在$Q$上恒成立，则：
		\begin{equation*}
		...
		\end{equation*}
		\item 设闭方块$Q=V\times W\subseteq\mathbb{R}^{n}\times\mathbb{R}^{p}$，闭方块$V\subseteq\mathbb{R}^{n}$，闭方块$W\subseteq\mathbb{R}^{p}$，函数$f(x,y):Q\to\mathbb{R}$，$x\in\mathbb{R}^{n},\;y\in\mathbb{R}^{p}$，$\underline{I}_W(\overline{I}_W)$是将$f(x,y)$看作$y$的函数在$W$上的下（上）积分。若$f$在$Q$上可积，则$\underline{I}_W$和$\overline{I}_W$在$V$上可积，且有：
		\begin{equation*}
			\int_{Q}f(x,y)\dif(x,y)=\int_{V}\underline{I}_W\dif x=\int_{V}\overline{I}_W\dif x
		\end{equation*}
	\end{enumerate}
\end{property}
\begin{proof}
	...
	即：
	\begin{equation*}
		\lim_{|P|\to0}\sigma(f,P,\xi)=\lim_{|P'|\to0}\sigma(f,P',\xi')
	\end{equation*}
	...
	(6)任取$V$和$W$上的分割$P_V,P_W$，则$P=\{P_V,P_W\}$构成$Q$的一个分割。 设$P_V$将$V$划分为$\seq{V}{N_V}$
\end{proof}
