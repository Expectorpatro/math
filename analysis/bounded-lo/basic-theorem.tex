...
\begin{proof}
	...
	\end{equation*}
	因为$E$有界，所以$\exists\;M>0,\;||y||\leqslant M$，于是$\forall\;y\in E,\;||T^{-1}y||\leqslant KM$，即$T^{-1}E$是有界集。由$E$的任意性，$T^{-1}$将任意有界集映成有界集，故$T^{-1}$有界。
\end{proof}
\subsubsection{等价范数与拓扑同构}
\begin{theorem}
	$X$是线性空间，$||\cdot||_1,\;||\cdot||_2$是定义在$X$上的两个范数，$X$按照这两个范数均为赋范线性空间。$(X,||\cdot||_1)$与$(X,||\cdot||_2)$拓扑同构的充分必要条件为$||\cdot||_1,\;||\cdot||_2$等价。
\end{theorem}
\begin{proof}
	设$T$为$X$上的单位算子，显然$T$是线性算子。\par
...
\end{corollary}
\begin{proof}
	设$T$为$X$上的单位算子，显然$T$是双射并且是线性算子。由条件中的不等式可知$T$有界，因此$T$连续。$(X,||\cdot||_2)$是Banach空间，所以$(X,||\cdot||_2)$是第二型集。因为$T$把$(X,||\cdot||_1)$映成第二型集$(X,||\cdot||_2)$且$T$是单射，由逆算子定理，$T^{-1}$存在且有界。易证$T^{-1}$也是线性算子，因此$T^{-1}$连续。综上，$(X,||\cdot||_1)$与$(X,||\cdot||_2)$拓扑同构，从而$||\cdot||_1,\;||\cdot||_2$等价。
\end{proof}
...
\begin{proof}
	...
	必要性：任取$x\in\overline{E}$，则$\exists\;\{x_n\}\in E$并且$\{x_n\}\to x$。因为$T$是有界线性算子，所以$T$连续，所以$\{Tx_n\}\to Tx$。由$X\oplus Y$中范数的定义，$\{(x_n,Tx_n)\}\to(x,y)$。因为$T$是闭算子，所以$(x,y)\in G_T$，即$x\in E$，故$E$是$X$的闭子空间。
\end{proof}
...
\begin{proof}
	...
	\begin{equation*}
		\tilde{T}(x,Tx)=x
	\end{equation*}
	显然$\tilde{T}$是满映射。若$\tilde{T}$不是单射，则$\exists\;x\in X$使得$\exists\;y_1,y_2\in Y,\;y_1\ne y_2,\;Tx=y_1,Tx=y_2$，那么$T$就不是映射，所以$\tilde{T}$是单射。于是$\tilde{T}$是双射。\par
	设$a$是数，$(y,Ty)\in G_T$。由$T$的线性性可推出：
	\begin{gather*}
...
\subsection{共鸣定理}
\begin{definition}
	设$X$是线性空间，$T$是定义在$E$上的泛函。若对任意的$x,y\in X$，有：
	\begin{equation*}
