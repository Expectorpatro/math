...
\end{definition}
\begin{definition}
	设$(X,\rho)$是度量空间，$E\subseteq X$。若$\delta_E<+\infty$或$E$包含在$X$中某一点的某一邻域内或$E$包含在$X$中任意一点的某一邻域内，则称$E$是$(X,\rho)$中的\gls{BoundedSet}。
\end{definition}
\begin{note}
	三者的等价性很直观：\par
...
\section{度量空间上的收敛点列}
\begin{definition}
	设$(X,\rho)$是度量空间，从正整数集$\mathbb{N}^+$到$X$的一个映射，相当于用正整数编号的$X$中的点$x_1,x_2,\cdots$被称为一个\gls{SeqOfPoints}，记为$\{x_n\}$。
\end{definition}
\begin{definition}
...
\input{analysis/metric-space/def-pset}
\input{analysis/metric-space/mapping}
\input{analysis/metric-space/normedLS}