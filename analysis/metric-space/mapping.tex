...
\subsection{映射的极限}
\begin{definition}
	设$(X,\rho_X)$和$(Y,\rho_Y)$都是度量空间，$T$是一个$E\subseteq X$到$Y$的映射，$x_0\in X,y\in Y$。若对任意的$\varepsilon>0$，存在$\delta>0$，使得对$E\backslash\{x_0\}$中一切满足条件$\rho(x_0,x)<\delta$的$x$，都有$\rho_Y(Tx,y)<\varepsilon$，则称$T$沿$E$趋于$x_0$时，$T$的极限为$y$，记为：
	\begin{equation*}
		\underset{E}{\lim_{x\to x_0}}Tx=y
	\end{equation*}
\end{definition}
\begin{theorem}[Sequential Characterization of Limits]\label{theo:SequentialCharacterizationOfLimits}
	设$(X,\rho_X)$和$(Y,\rho_Y)$都是度量空间，$T$是一个$E\subseteq X$到$Y$的映射，$x_0\in X,y\in Y$。$T$沿$E$趋于$x_0$时$T$的极限为$y$的充分必要条件为：对于任意满足$\{x_n\}\to x_0$的点列$\{x_n\}\subseteq E\backslash\{x_0\}$都有$\{Tx_n\}\to y$。
\end{theorem}
\begin{proof}
	...
	\textbf{充分性：}若此时$T$沿$E$趋于$x_0$时极限不为$y$，那么$\exists\;\varepsilon>0$，$\forall\;\delta>0$，都存在满足条件$\rho_X(x,x_0)<\delta$的点$x\in E\backslash\{x_0\}$使得$\rho_Y(Tx,Tx_0)\geqslant\varepsilon$，因此可取一个点列$\{x_n\}$，满足$\rho_X(x_n,x_0)<\dfrac{1}{n}$。注意到此时满足$\{x_n\}\to x$但不满足$T\{x_n\}\to y$，矛盾。
\end{proof}
\begin{property}\label{prop:MappingLimits}
	设$(X,\rho_X)$和$(Y,\rho_Y)$都是度量空间，$T$是一个$E\subseteq X$到$Y$的映射，则：
	\begin{enumerate}
		\item 若$T$沿某个集合趋于某一点时有极限，则极限唯一；
		\item 设$(Z,\rho_Z)$是度量空间，$f$是$F\subseteq Y$到$Z$的映射，$TE\subseteq F$。若$T$沿$E$趋于$x\in$时极限为
	\end{enumerate}
\end{property}
\begin{proof}
	(1)由\cref{theo:SequentialCharacterizationOfLimits}和\cref{prop:ConvergentSeqOfPoints}(1)立即可得。\par
	(2)
\end{proof}
\subsection{映射的连续性}
\begin{definition}
	设$(X,\rho_X)$和$(Y,\rho_Y)$都是度量空间，$T$是一个$X$到$Y$的映射，$x_0\in X$。若对任意的$\varepsilon>0$，存在$\delta>0$使得对$X$中一切满足条件$\rho(x,x_0)<\delta$的$x$都有$\rho_Y(Tx,Tx_0)<\varepsilon$，或对于任意满足$\{x_n\}\to x_0$的点列$\{x_n\}\subseteq X$都有$\{Tx_n\}\to Tx_0$，则称$T$在$x_0$处是\gls{continuous}。
\end{definition}
两个条件的等价性由\cref{theo:SequentialCharacterizationOfLimits}即可得到。
\begin{definition}
