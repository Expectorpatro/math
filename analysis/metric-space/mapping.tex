...
\begin{property}\label{prop:MappingLimits}
	...
	\begin{enumerate}
		...
		\item 设$(Z,\rho_Z)$是度量空间，$S$是$F\subseteq Y$到$Z$的映射，$x_0\in X$是$E$的聚点，$y\in Y$是$T(E\backslash\{x_0\})$的聚点，$T(E\backslash\{x_0\})\subseteq F\backslash\{y\}$，$z\in Z$。若$T$沿$E$趋于$x_0$时极限为$y$，$S$沿$F$趋于$y$时极限为$z$，则：
		\begin{equation*}
...
\begin{proof}
	...
	(2)由定义立即可得。\par
	(3)由映射极限的定义，$E\backslash x_0$中任意满足$\{x_n\}\to x_0$的点列$\{x_n\}$都有$\{Tx_n\}\subseteq F\backslash\{y\}$且$\{Tx_n\}\to y$，根据\cref{theo:SequentialCharacterizationOfFunctionLimits}可知$\{S(Tx_n)\}\to z$，于是结论成立。
\end{proof}
\subsection{映射的连续性}
\begin{definition}
