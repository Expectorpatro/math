...
\begin{property}\label{prop:RMap}
	...
	\begin{enumerate}
		...
		\item (Squeeze Theorem) 若存在$\delta>0$，使得对任意的$x\in U(x_0,\delta)\cap E\backslash\{x_0\}$都有如下之一成立，则对应的结论也成立：
		\begin{enumerate}
		...
		\end{equation*}
		则存在$\delta>0$，使得当$x\in U(x_0,\delta)\cap E\backslash\{x_0\}$时，$f(x)<g(x)$；
		\item 若存在$\delta>0$使得当$x\in U(x_0,\delta)\cap E\backslash\{x_0\}$时有$f(x)\leqslant g(x)$，且$\lim\limits_{\substack{x\to x_0 \\ E}}f(x)$和$\lim\limits_{\substack{x\to x_0 \\ E}}g(x)$都存在，则：
		\begin{equation*}
