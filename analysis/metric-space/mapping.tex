...
\subsection{值域为$\mathbb{R}^{},\mathbb{R}^{n},\overline{\mathbb{R}^{}}^n$的映射}
...
\begin{property}\label{prop:RMap}
	对于定义在度量空间$(X,\rho_X)$或$\overline{\mathbb{R}^{}}^m$上取值在$\mathbb{R}^{n}$中的函数$T$，有：
	\begin{enumerate}
		\item 若$T$趋于某点时有极限，则其极限唯一；
...
\end{property}
\begin{proof}
	(1)由\cref{theo:SequentialCharacterizationOfFunctionLimits}和
\end{proof}