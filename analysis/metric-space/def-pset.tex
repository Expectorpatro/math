...
\end{proof}
\subsubsection{紧性}
\begin{property}\label{theo:CompactRn}
...
\begin{proof}
	...
	\end{equation*}
	由\cref{theo:RnComplete}可知级数$\sum\limits_{n=1}^{+\infty}a_nb_n$收敛。
\end{proof}
...
\begin{property}\label{prop:ConvergentSeries}
	...
	\begin{enumerate}
		...
		\item 若$\sum\limits_{n=1}^{+\infty}a_n$绝对收敛，则重排后的级数$\sum\limits_{n=1}^{+\infty}a^{'}_n$也绝对收敛，并且二者值相等。重排的含义是把该序列中的所有项无重复、无遗漏地改变一个顺序重新排出来，用符号表示即为$\alpha^{'}_n=\alpha_{\varphi(n)}$，其中$\varphi$是一个从$\mathbb{N}^+$到$\mathbb{N}^+$的双射；
		\item 若$\sum\limits_{n=1}^{+\infty}a_n$条件收敛，则对任意的$\xi\in\overline{\mathbb{R}}$，都存在$\sum\limits_{n=1}^{+\infty}a_n$的一个重排级数$\sum\limits_{n=1}^{+\infty}a^{'}_n$，使得$\sum\limits_{n=1}^{+\infty}a^{'}_n=\xi$；
	\end{enumerate}
\end{property}
\begin{proof}
	...
	\begin{align*}
		...
		&=\sum_{n=1}^{+\infty}a_n
	\end{align*}
	第一行到第二行利用级数的线性运算，第二行到第三行利用(1)的结果，第三行到第四行再次利用级数的线性运算。\par
	(3)(1)对于$\xi\in\mathbb{R}$：\par
	令：
	\begin{equation*}
		p_n=\frac{|a_n|+a_n}{2},\;q_n=\frac{|a_n|-a_n}{2},\;n\in\mathbb{N}^+
	\end{equation*}
