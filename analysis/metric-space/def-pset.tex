...
\end{definition}
\begin{theorem}
	设$(X,\rho)$是一个度量空间，$E\subseteq X$。$E$的界点不是聚点就是孤立点。
\end{theorem}
\begin{proof}
...
\begin{property}\label{prop:PointSet}
	设$(X,\rho)$是一个度量空间，$E\subseteq X$，$A\cup B\subseteq X$，则：
	\begin{enumerate}
...
\begin{property}\label{prop:ConvergentSeries}
	...
	\begin{enumerate}
		...
		\item 若$\sum\limits_{n=1}^{+\infty}a_n$绝对收敛，则重排后的级数$\sum\limits_{n=1}^{+\infty}a_n'$也绝对收敛，并且二者值相等。重排的含义是把该序列中的所有项无重复、无遗漏地改变一个顺序重新排出来，用符号表示即为$\alpha_n'=\alpha_{\varphi(n)}$，其中$\varphi$是一个从$\mathbb{N}^+$到$\mathbb{N}^+$的双射；
...
\begin{proof}
	...
	\textbf{正项级数：}设级数$\sum\limits_{n=1}^{+\infty}a_n$为绝对收敛的正项级数，则级数$\sum\limits_{n=1}^{+\infty}a_n'$也是一个正项级数。因为：
	\begin{equation*}
