...
\begin{gather*}
	\left|R_i-R_j>Z_{\frac{\alpha^{\star}}{2}}\sqrt{b(k+1)k/6}\right| \\
	\alpha^{\star}=\frac{\alpha}{k(k-1)/2}
\end{gather*}
则可拒绝零假设。可以看出这是一个很保守的检验，$\alpha^{\star}$其实是做了多重假设检验的校正。\info{记得以后要写多重假设检验的校正问题}
\subsubsection{代码}
