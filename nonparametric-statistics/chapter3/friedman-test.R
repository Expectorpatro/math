friedman.test <- function(x, factor = NULL, block = NULL, 
  ...
  Q.call <- function(ranks, factors) {
    R_i <- tapply(ranks, factors, sum)
    12 / (b * k * (k + 1)) * sum(R_i^2) - 3 * b * (k + 1)
  }
  METHOD <- "Friedman rank sum test"
  k <- length(unique(factor))
  b <- length(unique(block))
  rank <- ave(response, block, FUN = rank)
  STATISTIC <- setNames(Q.call(rank, factor), "Q")
  TIES <- unlist(tapply(response, block, table))
  if (any(sapply(TIES, function(x) any(x > 1)))) {
    ties <- TRUE
  } else {ties <- FALSE}
  if (!exact || ties) {
    Qc <- STATISTIC / (1 - sum(TIES^3 - TIES) / (b * k * (k^2 - 1)))
      if (!correct) {
        p_value <- pchisq(Qc, k - 1, lower.tail = FALSE)
      } else {
        p_value <- pchisq(Qc - 0.5, k - 1, lower.tail = FALSE)
    ...
    Qs <- sapply(1:nrow(all_combinations), function(i) {
      b_combs <- possible_ranks[all_combinations[i, ], ]
      rank_i <- as.vector(b_combs)
      Q.call(rank_i, factor)
    })
    p_value <- mean(Qs >= STATISTIC)
  }
