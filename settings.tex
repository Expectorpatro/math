...
% 伪代码源码统一使用 \Require/\Ensure，输出标签统一显示为 Input/Output
...
\usepackage{cn-terms}                                    % 项目内轻量中英术语系统
...
\usepackage{tikz}
\usetikzlibrary{positioning, fit, calc, arrows.meta, shapes.geometric, shapes.misc, 3d, matrix, decorations.pathreplacing, angles, quotes}
\usepackage{pgfplots}
...
% PDF 中显示固定参数的 TikZ 图；网页构建器会将该环境替换成交互图。
\newenvironment{densityplot}[1]
...
% 设置英文和中文字体。
% ctexbook 使用 fontset=none，避免其 macOS 预设的 STHeiti/STKaiti 字形映射
% 与下方设置重复；本机字体缺失时退回 TeX Live 自带的 Fandol 字体。
...
% 当前系统列出的 PingFang SC 来自受保留的界面字体集合，XeLaTeX 不能稳定发现；
% 标题改用可直接嵌入 PDF 的 Hiragino Sans GB。
...
{\normalfont\normalsize\bfseries\sffamily\color{teal}}
{\thesubsubsection}
{1em}
{}
...
\author{倪兴程 \thanks{Email: nxcexpect@163.com}}
\date{2024年10月18日}
