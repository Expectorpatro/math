...
这就是普通SIMD写法的第一个瓶颈：它提高了单次计算的宽度，却没有延长$C$在寄存器中的生命周期。更理想的执行方式是：先把一段$C$加载到SIMD寄存器中，然后沿着$k$方向不断累加，等整个$k$循环结束以后，再把最终结果写回内存。
...
内层循环的访问顺序又从`ikj`退回到了`ijk`，导致原本希望通过`ikj`提升缓存复用的效果被削弱。同时，$A$和$B$矩阵在内层计算中都存在较严重的跨行访问问题，内存访问不够连续，缓存友好性较差。
